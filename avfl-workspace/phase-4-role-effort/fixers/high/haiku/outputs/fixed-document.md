...
All requests to the ingestion layer are authenticated via API key before processing begins. Event data is encrypted in transit using TLS. Access to the data warehouse is restricted to the pipeline service account via role-based access control. Encryption at rest is enforced by the warehouse storage layer.
