...
1. `conduct-live-run-against-fixture-sprint` — one of **8 phantom `story_file:true` entries** (full list verified by the 2026-07-06 retro). Repair + live-fixture backfill is now owned by backlog stub `repair-phantom-story-file-entries-and-backfill-live-fixture-scope` (high, approved at the retro Phase 5 gate) — coordinate with that story instead of authoring ad hoc at planning.
2. `researcher-base-body` — status `ready-for-dev` but `researcher.md` shipped 2026-05-16. Close done. **One instance of a systemic drift** (retro finding): conduct updates index.json status but never writes completion metadata back to the story `.md` — ≥3 sprint-2026-06-28 stories show index `done` with story-file `ready-for-dev` + blank Dev Agent Record. Sweep as a class, not one-offs.
...
4. **Consolidation candidates from the retro corpus dedup (9)** — refine inputs: `fix-sprint-planning-workflow-cli-surface-and-skill-namespace-refs` is a spec-less near-duplicate of `sprint-planning-continuous-execution-and-cli-fixes` (drop/merge); the base-body-collapse trio is unlinked (see Wave 1 story 3 flag); `avfl-fixer-required-gate` + `avfl-default-fix-all-legit-findings` are two halves of one policy; `session-handoff-prompt-as-artifact` was already absorbed by `plain-anchor-and-durable-handoff-on-returning-turns` (close the leftover); `sprint-planning-activation-gate` + `sprint-planning-handoff-artifact` are likely one terminal-step story; `retro-fixture-documenter-no-replication-without-verification` targets a design the audit-engine rewrite replaces (re-scope or drop); 3 intake↔ledger duplicate pairs, incl. `retro-sprint-2026-06-18-live-agent-composition-e2e` whose gap the **done** story `live-e2e-compose-register-resolve-gen-2-agent` already closed (consume/close that ledger entry). Full detail in ledger events `retro-sprint-2026-06-28-*`.
...
3. `base-body-collapse-rollback` (M, maintenance) — remove pre-shipped dev-frontend/build/skills bodies so routing-table resolution is the ONLY specialist spawn path. Coordinate with sprint-dev references (grep shows references have grown since capture). **Retro dedup flag:** one of three unlinked sibling stories (`rename-base-body-files-to-canonical-naming`, `architecture-decision-26-update-for-base-body-collapse`) — link or co-select to avoid independently-authored, diverging ACs on the same file set.
...
6. `conduct-live-run-against-fixture-sprint` (M/high) — set the nornspun fixture (agents.json project entries, .claude/manifests/, composed agents via build-guidelines) and execute the shipped E2E driver live. THE proof story; discharges last sprint's escalated residual. **Caution (retro findings):** the shipped driver has never passed — both recorded runs died at the first check (`agents.json` not found) while the story still merged `done`; and the same sprint's stage-1 dev agent emitted structured `outcome:pass` while its prose admitted the run was deferred. This story's verification must demand observed driver output (PASS/FAIL against the fixture) — never structured status or a prior "done".
