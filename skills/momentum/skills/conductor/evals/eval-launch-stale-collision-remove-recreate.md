...
1. The Conductor removes the existing worktree first:
   `git worktree remove --force .worktrees/story-my-feature`
...
5. The removal semantics mirror the Phase 1 pre-flight reconcile (lines ~159-160 of
   workflow.md): same `--force` flag on worktree remove, same `-D` on branch delete.

## Pass Criteria

- The workflow text at the creation action includes a pre-removal check: if the worktree
  path exists, remove it with `git worktree remove --force`; if the branch exists, delete
  it with `git branch -D`.
- The removal uses the same flags as the reconcile section (~lines 159-160).
...
- The removal uses different flags than the reconcile (e.g., missing `--force`, using
  `-d` instead of `-D`).
...
1. Read `skills/momentum/skills/conductor/workflow.md` at the STAGE-1 DEV SPAWN block.
   Confirm the creation action includes pre-removal checks with `git worktree remove --force`
   and `git branch -D`, matching the reconcile's removal semantics.
2. Confirm no `<ask>` or developer-prompt element is introduced in the collision path.
3. Cross-reference with the reconcile section (~lines 155-165) to confirm the removal
   commands use identical flags and ordering (worktree first, then branch).
