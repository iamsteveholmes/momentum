...
Large files (>50MB) are handled separately from the primary transformation path via an overflow queue. Overflow queue events are processed by the same transformation logic but batched at a reduced rate and routed to Stage 3 delivery via a dedicated overflow delivery channel. The overflow channel applies the same retry policy and SLA as the primary delivery path.
...
The pipeline exposes a metrics endpoint at `/metrics` with standard Prometheus counters for throughput, error rate, and queue depth. The monitoring layer runs continuously in parallel with all pipeline stages. The on-call team monitors the pipeline via the Prometheus dashboard.

Alert thresholds: error rate >1% triggers a P2 alert; queue depth >10,000 events triggers a P1 alert; throughput drop >50% from baseline triggers a P2 alert. SLA targets: 99.5% of events delivered within 60 seconds of ingestion under nominal load (95th-percentile benchmark: sub-60-second latency; 99.5th-percentile SLA commitment: ≤60 seconds). Escalation paths and runbook are maintained in the internal ops wiki at `wiki/pipeline-runbook`.
...
Benchmarks conducted on production-equivalent infrastructure (March 2026, 8-core nodes, 32GB RAM, 1Gbps network) show that batch processing achieves 3× higher throughput than record-by-record streaming for payloads under 10MB, driven primarily by reduced per-connection establishment overhead in the batch path. For larger payloads where memory pressure becomes a factor, streaming mode is preferred to avoid buffering costs.
