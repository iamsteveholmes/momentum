...
**Resolution recorded here:** `constitution-builder` emits **no `## Quick Routing` content** in the shared constitution under any write mode. All routing-table / diagnostic-table generation is delegated entirely to `agent-builder-skill` (the manifesto/Tier 2 layer), per DEC-038 and DEC-026 D4.

**Mapping to the two architecture-sanctioned options (architecture.md ~line 3061):**
This resolution corresponds to the spirit of **option (b)** — routing emission is parameterized per write-mode/role — taken to its logical conclusion: for the shared constitution write mode, the routing output is zero (no routing emitted). Option (a) (constitution emits project-universal routing) is **not adopted**: DEC-038 and DEC-026 D4 together establish that constitution-builder is narrowed to domain knowledge only, and even genuinely project-universal routing entries are the manifesto layer's responsibility to generate and own.

**FR136 reconciliation:** PRD FR136 describes the constitution layer as carrying a "quick routing table derived from the wiki KB." This story's resolution supersedes that literal reading: DEC-038 (a post-FR136 architectural decision) relocated per-agent routing to the manifesto layer. The reconciliation is that FR136's "quick routing table" referred to what is now the per-agent manifesto, not the shared constitution. The architecture decisions (DEC-038, DEC-026 D4) are the authoritative sources; FR136's wording should be treated as pre-DEC-038 phrasing that has been superseded. **Spec-impact note:** FR136 in `prd.md` should be amended to reflect that the constitution layer carries domain knowledge only; the diagnostic routing table is a per-agent manifesto concern. This amendment is a cross-artifact finding (see story completion notes).
