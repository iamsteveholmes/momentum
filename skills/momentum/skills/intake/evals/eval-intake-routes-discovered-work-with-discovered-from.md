...
`.momentum/beads-id-map.json` using the source story slug. The existing `practice-ledger.jsonl`
...
2. Intake appends a `created` event to `practice-ledger.jsonl` (unchanged from pre-A4 behavior,
   now using `event_type: created` per DEC-033 schema).
...
- `practice-ledger.jsonl` has the new entry with `event_type: "created"` (additive, not replaced).
