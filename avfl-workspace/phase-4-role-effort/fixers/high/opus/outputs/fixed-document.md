...
Authentication for inbound requests is handled at the ingestion layer (Stage 1). All inter-service communication uses encryption in transit (TLS). Data at rest in the data warehouse is encrypted using the warehouse provider's default encryption. Access to the metrics endpoint and operational tooling is restricted to the on-call team via role-based access control.
