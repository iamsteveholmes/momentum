...
Given a `_bmad-output/planning-artifacts/features.json` with at least 3 feature entries, and a `.momentum/stories/index.json` that contains at least one story that is neither dropped nor done and is not assigned to any feature slug present in features.json, the skill should:
