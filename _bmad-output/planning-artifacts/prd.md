...
editHistory:
  - date: '2026-05-18'
    changes: 'sprint-2026-05-17 spec impact (enforced-verification-rule, momentum-harnessjson-schema, e2e-validator-agent-body-rewrite, sprint-planning-frozen-per-story-contract, create-story-method-selection-step): added FR131 (Enforced Verification Standard Rule — plugin ships verification-standard.md rule file written by Impetus on first run; change-type routing table, prohibition on method substitution, harness-profile reference mandate, adversarial anti-insider-knowledge guard, cascade order non-overridable at lower scopes; docs/process/acceptance-testing-standard.md retired with forwarding pointer); modified FR5 (Impetus rule-write manifest includes verification-standard.md in momentum-versions.json, written to ~/.claude/rules/ on first run, following existing rule entry schema); added FR132 (Verification Harness Profile — momentum/verification-harness.json sibling to momentum/agents.json; top-level keys defaults/project; defaults declares env, execution_surfaces, drivers, platform_targets, human_review_carveouts, trivial_smoke_escape; plugin defaults opt everything out; projects override in project array; sole authorized writers agent-builder/agent-guidelines); modified FR74 (E2E Validator is method-polymorphic — accepts .eval.yaml/.trigger.md/.smoke.sh/.review.md/.feature contract formats, dispatches by file extension; environment startup/driver binding/human-review carveouts/trivial-smoke escape resolved from verification-harness.json at startup; BLOCKED if harness absent; no stack-specific references in agent body); modified FR58 (expanded to cover all 5 contract formats: .eval.yaml/.trigger.md/.smoke.sh/.review.md/.feature; format determined by story highest-tier change-type; dev agents blocked from all contract file types; verifier agents read from specs path exclusively); added FR133 (Story Verification Method Selection — momentum:create-story includes method-selection step after change-type classification, before Momentum Implementation Guide injection; loads verification-standard.md, maps change types to methods, resolves single method when types converge, escalates on non-subsumable conflicts, writes verification_method to story YAML frontmatter; halts with diagnostic if verification-standard.md absent; unclassified-only stories escalate rather than default silently); added FR134 (Frozen Verification Contracts and Holistic Coverage Plan — sprint-planning Step 3.5 runs three phases: contract authoring per story in specs/, coverage-plan.md at sprint root, adversarial guard via decorrelated adversarial agent applying Outsider Test; failing clauses rewritten and re-checked up to 2 passes; residual failures surfaced to developer before activation); added FR135 (Sprint Activation Contract Gate and Post-Activation Immutability — momentum-tools sprint activate fails if any story lacks contract in specs/ or coverage-plan.md is absent; after activation specs/** and coverage-plan.md are hook-immutable; dev agent Read also blocked for specs/).'
  - date: '2026-05-03'
    changes: 'sprint-2026-05-03 spec impact (momentum-canvas): deprecated FR104 (momentum:feature-status superseded by momentum:canvas — delivery changes from static file:// artifact to live Hono+Bun server on port 3456); updated FR106 (static self-contained HTML model superseded by live Hono+Bun server; layout requirements migrate to server-rendered routes / and /lenses/features); updated FR105 (distinguished LLM-based plausibility check from deterministic structural heuristic in canvas /lenses/features route — has_gap=true when stories_done === 0 AND status !== working; canvas route cannot invoke LLMs); updated FR107 (cache writer changes from momentum:feature-status to momentum:canvas following skill deprecation; cache file path and Impetus read behavior unchanged); added FR125 (momentum:canvas skill — port-3456 Hono+Bun server, cmux services pane via respawn-pane, cmux viewer pane, persists across sessions, supersedes feature-status static-file approach); added FR126 (live HTMX dashboard polling — features lens 2s, cycle timeline 5s, sprint lens 2s, updates innerHTML without page reload); added FR127 (Sprint Lens — active sprint identifier, start date, retro completion state, empty-state, story detail view with Blocked/In Progress/Validated bands); added FR128 (Cycle Timeline Lens — 7-node horizontal timeline, done/next-required/not-run/pending node states, boundary from retro_run_at); added FR129 (Feature L2 Drill-Down — reading-mode detail view, Source Serif 4 at 65ch/18px/1.70, warm light cross-fade, HTMX hx-push-url breadcrumb OOB swap); added FR130 (Story L3 Drill-Down — story file rendering, JetBrains Mono frontmatter strip, collapsible dev notes, context-aware breadcrumb).'
  - date: '2026-04-28'
    changes: 'sprint-2026-04-27 spec impact (plugin-cache-staleness-detection, impetus-momentum-state-migration, sprint-planning-adds-per-story-approval-gate, harden-sprint-dev-phase5-spawn-prompts, retro-team-singleton-guard, fix-retro-documenter-replication-defect, retire-sprint-log-final-cleanup, retro-transcript-extraction-hardening): added FR119 (Impetus session-start preflight detects plugin cache vs source-tree plugin.json version skew, surfaces prominent warning with both versions and remedy `/plugin marketplace update momentum` + fresh session restart, tolerates failure modes silently, adds `momentum-tools session plugin-cache-check` subcommand); added FR120 (`.momentum/signals/` ledger directory — read-only pending-work-flags ledger consumed by Impetus orientation; schema signal_type/origin/created/payload/optional cleared; initial recognized types triage-uncleared and avfl-finding-pending-upstream-fix; writers momentum:retro / momentum:triage / momentum:avfl; closes DEC-011 Gate G1 for canvas redesign); added FR121 (per-story approval gate at sprint-planning Step 3 — three-option Approve/Revise/Reject prompt via cmux markdown viewer; persistent approval records in sprints.planning.approvals[] with story_slug/approved_at/decision/story_file_sha; new momentum-tools sprint story-approve and verify-approvals subcommands; activation blocked unless every story has approved entry with current matching SHA; approvals carry through planning → active → completed); added FR122 (defense-in-depth prohibition on service-state pre-announcement and offering pytest as substitute for live testing in sprint-dev Phase 5 reviewer spawn block; QA Reviewer agent definition gains Environment Prerequisites + Critical Constraints — live-test execution, AC-evidence-against-execution, MISSING vs BLOCKED disambiguation — parallel to E2E Validator); added FR123 (retro Phase 4 post-TeamCreate singleton-count guard reads team config and asserts exactly 1 documenter + 1 each of auditor-human/auditor-execution/auditor-review (4 total, no duplicates, no surplus); HALTs with diagnostic on failure; no continue-anyway path); added FR124 (retro Phase 4 team-composition topology — documenter declared as singleton coordinator distinct from auditor fan-out group; closes RV-05 / E14 / E15 replication defect); modified FR54 (added staleness-warning step as one explicit exception to silent-read rule); modified NFR21 (note: new staleness check absorbs into the single startup-preflight call — no additional Bash subprocess calls); modified FR66 (REMOVED secondary-evidence-source sentence about agent JSONL logs — DuckDB transcript audit is sole evidence source per ARCH-5/Decision 27; added Phase 2 hard-fail behavior on zero-session extraction; added 4-agent auditor team composition with documenter as singleton coordinator not TeamCreate fan-out role; auditors fan out as 3 individual Agent spawns each carrying documenter handle); modified FR87 (transcript-query.py hardened on six dimensions — worktree path discovery via `git worktree list --porcelain`, UTC end-of-day inclusivity at --before, json.dumps-only serialization, dynamic script-path resolution from `~/.claude/plugins/cache/momentum/momentum/*/scripts/transcript-query.py`, --story-slugs filter for same-day sprints, peek-first large-file convention propagated to retro auditor/documenter prompts and sprint-planning Step 4.5 spec-impact-discovery agents); modified FR91 (strengthened with path resolution + slug-filter accuracy callouts); modified FR55, FR58, FR68, FR108, FR115, FR117 (paths updated to `.momentum/...` with cross-reference to FR120); modified FR59 + FR62 (added per-story approval gate hooks per FR121); modified FR74 (added QA Reviewer parallel contract to E2E Validator — must follow project `.claude/rules/e2e-validation.md` Environment Startup, treat spawn-prompt service-state context as context not permission to skip, report BLOCKED not MISSING on missing infrastructure; same agent-definition self-sufficiency contract).'
  - date: '2026-04-26'
    changes: 'momentum:refine drift audit (8 findings applied, no requirement language removed): (1) FR43 upstream-fix — annotated story-dropped, capability reframed through momentum:distill (FR97) + retro signal classification (FR96); standalone momentum:upstream-fix skill exists in plugin but is not workflow-invoked. (2) FR39–FR41 story-cycle — added section-level backlog scope note documenting 5 dropped stories from story-cycles epic (full-story-cycle, create-story-and-dev-story, code-reviewer-adversarial, upstream-fix, dev-story); replacements landed via sprint redesign (create-story + dev + sprint-dev + quick-fix). (3) FR12–FR17 provenance — added section-level backlog status note: all 5 artifact-provenance stories remain backlog; "Provenance catches stale sources" first-value signal not yet realized in implementation; requirement preserved. (4) FR22 authority-hierarchy — confirmed drop is correct (native Claude Code behavior); appended audit-confirmation note. (5) FR28–FR33 flywheel — added section-level backlog status note: all 5 practice-compounds stories remain backlog; momentum:retro (FR66/FR96) and momentum:distill (FR97) partially substitute; cross-story ledger + pattern detection + fix-ratio metric still missing. (6) FR34–FR38 protocol-substitution — added section-level backlog status note: entire bring-your-own-tools epic is backlog; Journey 3 not currently exercisable end-to-end. (7) FR102–FR113 feature-orientation — added section-level backlog status note: schema + grooming + status + cache + sprint-summary + breakdown shipped; Canvas redesign and HTML-directory dashboard still backlog. (8) FR91 transcript-error-detection — strengthened annotation: structural-detection portion is implemented (transcript-query-calibration done); the <5% FP rate empirical validation standard is explicitly deferred without a covering story.'
  - date: '2026-04-14'
    changes: 'sprint-2026-04-14 spec impact (triage-skill + retro-triage-handoff stories, DEC-005 + DEC-007): modified FR100 (intake now also captures feature_slug per DEC-005 D1 and story_type per DEC-005 D5 — feature | maintenance | defect | exploration | practice — via features.json-backed suggestion flow; momentum-tools sprint story-add CLI extended with --feature-slug and --story-type flags); annotated FR49 with superseded-partial by FR114 notice (raw-input → epics.md-mutation contract replaced; triage-inbox.md file input retired); annotated FR52 replacing the triage-inbox.md retro-output reference with a DEC-007 retirement note and forward reference to FR116 intake-queue.jsonl handoff events; added new "Triage and Unified Intake Queue" subsection with four new FRs — FR114 (momentum:triage multi-item batch classification skill with six-class taxonomy ARTIFACT/DISTILL/DECISION/SHAPING/DEFER/REJECT, batch-approval UX, delegation to intake/distill/decision, session-start re-surfacing of open shape/defer items), FR115 (unified intake-queue.jsonl event log at _bmad-output/implementation-artifacts/intake-queue.jsonl with source and kind discriminators, base and optional schema, single source of truth for non-story triage outcomes and retro → planning handoff, supersedes never-built intake-queue.jsonl master-plan contract AND never-built triage-inbox.md architecture contract per DEC-007, CLI-only writes), FR116 (retro Phase 5 third disposition — carry forward as handoff event — writes source: retro / kind: handoff / status: open events with optional feature_state_transition and failure_diagnosis enrichment fields per DEC-005 D7/D8, no value-floor gap-check at handoff per D10, developer-approval preserved), FR117 (sprint-planning Step 1 Phase A.5 extended to read open handoff entries, Phase C output gains "Open handoff items from recent retros" labeled section, defensive empty-queue handling).'
  - date: '2026-04-11'
    changes: 'Feature-orientation epic spec impact (sprint-2026-04-11): added FR102 (features.json artifact schema), FR103 (feature type taxonomy — flow/connection/quality), FR104 (momentum:feature-status standalone skill with HTML artifact and cmux browser pane), FR105 (coverage gap analysis per feature), FR106 (HTML artifact layout — summary stats, Mermaid graph, type-grouped tables, GAP rows sorted to top), FR107 (Impetus feature-status cache — four greeting states, zero startup tool calls), FR108 (sprint boundary summary — retro Phase 6 spawns feature-status, writes sprint-summary.md, sprint planning Step 1 reads it), FR109 (practice project rendering path — skill topology and SDLC coverage table); updated NFR20 (note extended by NFR21); added NFR21 (startup performance constraint — no additional Bash subprocess calls, one-call happy path); annotated momentum:status repo structure entry as superseded by FR104.'
  - date: '2026-04-11'
    changes: 'Removed /momentum:create-epic and /momentum:develop-epic — superseded by momentum:create-story + momentum:epic-grooming + sprint model (developer decision 2026-04-11): replaced FR50 with single-line removal notice; replaced FR51 with single-line removal notice; rewrote FR52 to reflect current lifecycle (triage → sprint-planning → sprint-dev → retro → triage) without create-epic lock step; updated FR41 to remove dual-model description — sprint model is the sole execution model; updated Growth Features section to remove create-epic/develop-epic references.'
  - date: '2026-04-11'
    changes: 'Drift audit sync: superseded FR50 (/create-epic replaced by create-story + epic-grooming + sprint model); updated FR49 (downstream step is sprint-planning not create-epic); updated FR52 (create-epic lock step noted as superseded); updated FR6 (current primary menu items are sprint-based; removed /create-epic and /develop-epic as top-level items; added /momentum:assessment, /momentum:decision, /momentum:intake); updated Growth Features section to reflect same; added FR88 covering-story-dropped annotation (code-reviewer-skill-performs-adversarial-review dropped, sprint-dev integration gap unresolved); added FR98 (/momentum:assessment — ASR document generation); added FR99 (/momentum:decision — SDR document generation); added FR100 (/momentum:intake — lightweight story capture); added FR101 (plugin developer E2E validation workflow via CMUX); added FR91 validation standard not-yet-implemented annotation; cleaned up struck-through FR56, FR57, FR89 to single-line removal notices; added FR61a partial-implementation annotation (path-scoped rules confirmed, AVFL checkpoint coverage unverified).'
  - date: '2026-04-08'
    changes: 'Sprint 2026-04-08 spec impact: removed FR56 (agent observability — journal write infrastructure deleted, DuckDB transcript audit is sole evidence source); removed FR57 (fault-tolerant logging — same rationale as FR56); updated FR85 (spawn deduplication observability via DuckDB transcript audit instead of momentum-tools log, spawn registry FR84 unchanged); removed FR89 (SubagentStart/SubagentStop hooks — subagent lifecycle observable via DuckDB transcript audit); added FR95 (quality gate parity — all execution paths apply equivalent gates, momentum:dev is internal-only, single-story uses quick-fix); modified FR53 (momentum:dev is internal-only, not user-invocable from Impetus, called by sprint-dev and quick-fix); modified FR58 (spec quality pre-check after Gherkin generation validates structure, outsider-test compliance, template conformance before dev spawn); modified FR40 (E2E Validator untestable-scenario findings tagged with spec-quality metadata, surfaced in dedicated retro section for pattern aggregation).'
  - date: '2026-04-07'
...
- **Impetus as Sprint/Epic Orchestrator** — Impetus evolves from story-level guidance to epic-level and sprint-level orchestration: `triage` workflow for backlog shaping, `/momentum:create-story` for individual story creation, `/momentum:epic-grooming` for epic taxonomy management, `/momentum:sprint-planning` for cross-epic story selection and team composition, `/momentum:sprint-dev` for dependency-driven execution of sprint stories. The epic remains the primary unit for grouping and scoping work; the sprint is the primary unit for execution. Impetus dispatches agents, never implements. Human touchpoints at merge gates and critical AVFL findings only (see FR49, FR52–FR54, FR59–FR70)
...
│   │   └── SKILL.md
│   ├── status/             ← /momentum:status [superseded — see FR104: momentum:feature-status replaces this skill]
│   │   └── SKILL.md
...
> *Backlog status (2026-04-26): All five FR12–FR17 stories remain in the `artifact-provenance` epic with status `backlog` — `derives-from-frontmatter-establishes-traceability`, `provenance-scanner-builds-referenced-by-graph`, `hash-based-staleness-detection-for-internal-documents`, `time-based-staleness-for-edge-documents`, and `claim-level-provenance-status-and-integrity-enforcement`. The "Provenance catches stale sources" first-value signal stated under Success Criteria is therefore not yet realized in implementation. Manual `derives_from` frontmatter authoring exists in some artifacts (this PRD declares it) but no scanner, no auto-generated `referenced_by` graph, no hash-based or time-based staleness detection, and no claim-level provenance status enforcement is shipped. Requirement language is preserved here because the intent stands; the implementation gap is intentionally surfaced rather than removed.*
...
- **FR22:** ~~DROPPED~~ — Authority hierarchy rules auto-loading via `.claude/rules/` is inherent to Claude Code's native behavior (rules in `.claude/rules/` auto-load in every session including subagents). No Momentum-specific implementation is required. Story explicitly dropped during backlog refinement *[Confirmed resolved during 2026-04-26 refine drift audit — drop is correct, native Claude Code behavior covers it.]*
...
- **FR131: Enforced Verification Standard Rule** — The plugin ships a rule file at `skills/momentum/references/rules/verification-standard.md` written by Impetus to `~/.claude/rules/` on first run. The rule defines: (a) a change-type → verification-method routing table; (b) prohibition on method substitution without written justification in the story's frozen contract; (c) mandate that every verified change declare a harness-profile reference in `momentum/verification-harness.json`; (d) adversarial anti-insider-knowledge guard rejecting contract clauses verifiable only with source code access; (e) cascade order: global → project → path-scoped, with routing table and adversarial guard non-overridable at lower scopes. `docs/process/acceptance-testing-standard.md` is retired with a forwarding pointer. **Priority: High** *(Enforcement & Quality Governance)*
...
- **FR49:** `triage` workflow accepts raw input — conversational or file-based (`triage-inbox.md`) — and produces mutations to `epics.md`: story titles, one-line scope, epic assignments, priority ordering. Only unlocked epics (those without story files already created) are mutable. Cross-epic dependency violations are surfaced before committing changes. Triage is repeatable — it may be run any number of times; the downstream step is sprint planning via `/momentum:sprint-planning`. **Priority: High** *[Superseded-partial by FR114 — the multi-item batch classification `momentum:triage` skill replaces the raw-input → `epics.md`-mutation contract described here. The `triage-inbox.md` file input is retired per DEC-007 (FR115 unified `intake-queue.jsonl`). This entry is retained for historical traceability.]*
...
- **FR52:** Epic lifecycle: the epic is the primary unit for grouping and scoping work; the sprint is the primary unit for execution. The lifecycle is: **triage** (mutable, repeatable backlog shaping) → **sprint-planning** (cross-epic story selection, team composition) → **sprint-dev** (dependency-driven execution) → **retro** (structured retrospective — `triage-inbox.md` retired per DEC-007; retro writes `intake-queue.jsonl` handoff events, see FR116) → **triage** (next cycle begins). Stories are created individually via `momentum:create-story`, managed via `momentum:epic-grooming`, and executed via the sprint model. If a blocker or scope change occurs mid-sprint, the sprint is closed with status `done-incomplete` and incomplete stories are re-triaged into the next cycle. **Priority: High**
- **FR53:** `momentum:dev` is a pure internal executor: worktree setup, bmad-dev-story invocation, agent logging, and structured completion signal — no AVFL invocation, no status transitions, no DoD supplement, no code review offer. AVFL and status transitions are handled by the orchestration layer (`momentum:sprint-dev` / `momentum:quick-fix`). The completion output emits a structured JSON signal (status, files modified, test results) that the caller can parse. `momentum:dev` is internal-only — called by `momentum:sprint-dev` and `momentum:quick-fix`. It is not user-invocable from Impetus. Single-story development uses `/momentum:quick-fix`. The merge gate still requires developer confirmation. This subsumes the original `momentum-dev-auto` intent — the base `momentum:dev` is itself the stripped-down executor. **Priority: High**
...
- **FR54: Session Orientation — Narrative Greeting** At session open, Impetus reads `stories/index.json` and `sprints/index.json` and renders a narrative prose greeting as the first visible output. The greeting is pure prose — no progress bars, no fill bars, no story counts, no slug lists, no wave displays. Each greeting state produces a short narrative paragraph describing the current situation, an optional second paragraph for the planned sprint context, and an adaptive menu of 3-4 items appropriate to the current state. The greeting opens with the word "Momentum" as a header, followed by the narrative body and menu, and closes with a one-line forward-motion phrase. Nine greeting states are defined:
  ...
  **Menu structure rules:** "Plan a sprint" appears when no planned sprint exists OR when the planned sprint status is still "planning." It is hidden when the planned sprint status is "ready." "Run retro" / "Run retro" appears only when the active sprint status is "done." "Finish planning" appears only when a planned sprint exists in "planning" status. "Create story" and "Generate guidelines" are sub-workflows of sprint planning and backlog refinement — they are never top-level menu items.

  **Plugin cache staleness warning (one explicit exception to silent-read rule):** Per FR119, the startup-preflight read includes a plugin cache version vs source-tree `plugin.json` version comparison. When skew is detected, Impetus surfaces a prominent warning naming both versions and the remedy (`/plugin marketplace update momentum` + fresh session restart) BEFORE the narrative greeting. This is the one explicit exception to the silent-read rule for session orientation — version skew between active plugin cache and source-tree must be visible because operator-discipline mitigation depends on the developer seeing the mismatch. All other failure modes (no-cache, no-source, indeterminate states) are tolerated silently. **Priority: High** *(Epic X — Impetus UX Redesign)*

- **FR119: Plugin Cache Staleness Detection** — Impetus session-start preflight detects skew between the active Claude Code plugin cache version (the version of the Momentum plugin currently loaded by Claude Code) and the source-tree `plugin.json` version (the version declared in the Momentum repo on disk). When skew is detected, Impetus surfaces a prominent warning naming both versions and the remedy: run `/plugin marketplace update momentum` and start a fresh session. The warning appears BEFORE the narrative greeting (FR54) — it is the one explicit exception to the session-orientation silent-read rule. The check is implemented as a new `momentum-tools session plugin-cache-check` subcommand whose output is read inline within the existing startup-preflight Bash call (preserving NFR21's one-call ceiling — see NFR21 note). Failure-mode tolerance: the check is silent on no-cache (plugin cache absent), no-source (source-tree `plugin.json` absent), and indeterminate states (e.g., dev-mode loads, malformed manifests) — only true skew (both versions resolvable, both versions different) surfaces a warning. This is operator-discipline mitigation: Momentum cannot prevent stale-cache regressions automatically, but it can make the staleness visible so the developer can act. **Priority: High** *(Session Orientation — Impetus)*

### Evaluation Flywheel

> *Backlog status (2026-04-26): The core flywheel mechanism (findings ledger, cross-story pattern detection, flywheel workflow with developer-consent gates, upstream fix application across levels, fix-level ratio tracking) is described in this PRD as the "sustained value" outcome — but FR28–FR33's covering stories in the `practice-compounds` epic are still `backlog`: `findings-ledger-accumulates-quality-findings-across-stories`, `cross-story-pattern-detection-surfaces-systemic-issues`, `flywheel-workflow-explains-issues-and-guides-upstream-trace`, `developer-consent-required-at-every-flywheel-step`, and `practice-health-metric-and-fix-level-tracking`. Two adjacent capabilities have shipped and partially substitute: `momentum:retro` writes structured findings (FR66, FR96) using the same `signal_type` taxonomy described in FR28, and `momentum:distill` (FR97) applies upstream fixes immediately for Tier 1 findings. What is missing is the cross-story ledger accumulation, automated pattern detection across sprints, and the FR33 fix-ratio metric. Requirement language is preserved here because the intent stands; the implementation gap is intentionally surfaced rather than removed.*

- **FR28:** Findings ledger can accumulate findings across stories with category, root cause classification, and upstream level. Each finding carries two orthogonal classification fields: `signal_type` (causal: `Context | Instruction | Workflow | Failure`) and `behavioral_type` (behavioral: `correction | redirection | frustration | praise | decision`). The `signal_type` field is populated by the retro documenter and by `momentum:distill`; it drives routing to the correct artifact destination. Each finding also carries `origin` (`flywheel | distill`) to distinguish code-review-origin findings from practice-distillation-origin findings for FR33 ratio tracking.
...
> *Backlog status (2026-04-26): The bring-your-own-tools / protocol-substitution capability described in FR34–FR38 is entirely unimplemented. All four covering stories in the `bring-your-own-tools` epic are `backlog`: `project-configuration-file-defines-protocol-bindings`, `protocol-gap-resolution-creates-valid-config-entries`, `workflow-steps-resolve-through-protocol-interfaces-at-invocation-time`, and `protocol-substitution-satisfies-interface-contract`. Today, workflows reference specific Momentum skills directly (e.g., `momentum:dev`, `momentum:avfl`, `momentum:code-reviewer`) — there is no project-level protocol-to-implementation mapping file, no gap-resolution conversation in Impetus, and no runtime indirection through a protocol registry. The conceptual architecture (Protocol-Based Integration Architecture section earlier in this PRD) stands as design intent. Journey 3 ("Something Doesn't Fit" — Kotlin substitution) cannot currently be exercised end-to-end. Requirement language is preserved here because the intent stands; the implementation gap is intentionally surfaced rather than removed.*
...
- **FR132: Verification Harness Profile** — The plugin ships `momentum/verification-harness.json` as a sibling to `momentum/agents.json`. Top-level keys: `defaults` (plugin-shipped settings) and `project` (initially-empty array, written by `agent-builder`/`agent-guidelines`). The `defaults` block declares: `env` (startup command + readiness probe), `execution_surfaces` (change-type → surface name or `'skip'`), `drivers` (cmux/skill/maestro/playwright/curl with `enabled` boolean), `platform_targets` (macos/android/ios/web with `enabled` boolean), `human_review_carveouts` (array of change-type slugs), `trivial_smoke_escape` (enabled boolean + applies_to array). Plugin defaults opt everything out — all surfaces `'skip'`, all drivers/platforms disabled. Projects override in the `project` array. Sole authorized writers: `agent-builder`, `agent-guidelines`. **Priority: High** *(Protocol-Based Integration)*

### Specification & Development Workflow

> *Backlog scope note (2026-04-26): Five stories from the original `story-cycles` epic were dropped during refinement — `full-story-cycle-guided-by-impetus`, `create-story-and-dev-story-skills-active`, `code-reviewer-skill-performs-adversarial-review`, `upstream-fix-skill-analyzes-quality-failures`, and the legacy `dev-story-skill` story. Replacements landed via the sprint redesign: `momentum:create-story` (FR — see Tactical Workflows section), `momentum:dev` (FR53), `momentum:sprint-dev` (FR62/FR63), and `momentum:quick-fix` (FR78) collectively deliver the story-cycle capability described in FR39–FR42. Adversarial code review per story is captured in FR88 but its covering story was also dropped — that gap is acknowledged inline at FR88. Upstream-fix coverage is reframed at FR43.*
...
- **FR40:** ATDD workflow can generate failing acceptance tests from Gherkin criteria before implementation begins. E2E Validator findings about untestable scenarios are tagged with `spec-quality` metadata and surfaced in a dedicated retro section for spec quality pattern aggregation — enabling systematic upstream improvement of Gherkin spec authoring
- **FR41:** Developer can complete a full story cycle guided by the orchestrating agent: spec review → ATDD → implement → review → flywheel. The sprint model is the execution model: stories are pulled from across epics during sprint planning, executed via dependency-driven concurrency by `momentum:sprint-dev`. Individual story cycles are invoked by Impetus; the developer interacts at sprint granularity *[The "full story cycle" capability is delivered through the combination of `momentum:create-story` + `momentum:sprint-planning` + `momentum:sprint-dev` + `momentum:retro` rather than a single end-to-end skill — the original `full-story-cycle-guided-by-impetus` story was dropped in favor of this sprint-model decomposition.]*
- **FR42:** System can track visual progress through the story cycle, always showing current phase and next phase
- **FR43:** Developer can invoke the upstream fix skill to analyze a quality failure and propose corrections at the appropriate upstream level *[Backlog story `upstream-fix-skill-analyzes-quality-failures` was dropped. The capability is partially absorbed into `momentum:distill` (FR97 — practice artifact distillation classifies fix scope and applies upstream changes to rules/references) and the retro signal classification flow (FR96 — causal `Context | Instruction | Workflow | Failure` taxonomy routes findings to the correct upstream artifact). A standalone `momentum:upstream-fix` skill exists in the plugin but is not currently invoked by any workflow path. The "trace failure to spec/rule root cause" intent is preserved through these two skills; a dedicated upstream-fix skill is deferred until a use case emerges that distill + retro do not cover.]*
- **FR133: Story Verification Method Selection** — `momentum:create-story` includes a method-selection step inserted after change-type classification and before Momentum Implementation Guide injection. The step loads `verification-standard.md`, maps each classified change type to its routed method, resolves a single method when change types converge (`specification` tasks always subsumed by the story's primary method), escalates to the developer on non-subsumable conflicts, and writes `verification_method: <selected>` to the story's YAML frontmatter. If `verification-standard.md` is absent, the step halts with a diagnostic message. Unclassified-only stories escalate rather than defaulting silently. **Priority: High** *(Specification & Development Workflow)*
...
> *Backlog status (2026-04-26): Feature Orientation is partially shipped — implementation is split across the `feature-orientation` epic (feature-status skill, Impetus cache, sprint-boundary summary, practice-path rendering, feature-grooming bootstrap+refine, value-analysis requirements, developer-interaction safety) and the `practice-compounds` epic (feature-status HTML-directory dashboard refresh, retire-stub follow-ups). Done as of this audit: `feature-artifact-schema-define-features-json-and-populate-initial-instances`, `feature-status-skill`, `sprint-boundary-context-compression-structured-sprint-summary-at-retro-close`, `impetus-feature-status-cache`, `feature-status-practice-path`, `feature-grooming` (covering FR102–FR108, FR109, and FR110–FR113 — bootstrap + refine + value-analysis + developer-interaction). Backlog: `feature-dashboard-ux-wireframes-hierarchical-drill-down`, `story-level-feature-dependency-graph-ux-wireframes`, `feature-status-html-directory-dashboard`, the seven `canvas-*` phases (Vite scaffold through reading-mode polish), and `retire-feature-status-html-directory-dashboard-orphan-stub`. FR118 (`momentum:feature-breakdown`) is shipped — the skill exists and is registered. Net: schema + grooming + status + cache + sprint-summary + breakdown are functional today; the next-generation interactive dashboard (Canvas) is unstarted.*

- **FR102: Feature Artifact Schema** — The practice must maintain a `features.json` artifact at `_bmad-output/planning-artifacts/features.json` that tracks user-observable capabilities as persistent planning units. Each feature entry includes: `feature_slug` (kebab-case unique identifier), `name` (human-readable display name), `type` (one of `flow | connection | quality` — see FR103), `description` (narrative description of the capability), `value_analysis` (multi-paragraph string, required — three paragraphs: current value delivered, full vision including capabilities beyond pain removal, and known gaps), `system_context` (string, required — how the feature fits and enhances the overall product), `acceptance_conditions` (array of strings, required — each entry formatted as "A developer can [action] and [observe outcome]"), `status` (one of `working | partial | not-working | not-started`), `prd_section` (PRD anchor referencing the functional requirement(s) that cover this feature), `stories` (array of story slugs assigned to deliver this feature), `stories_done` (count of completed stories), `stories_remaining` (count of incomplete stories), `last_verified` (ISO date of last status check), `notes` (free-text). Authorized writer: `momentum:feature-grooming`. This file is not written by sprint-dev, momentum-tools, or any other skill. **Priority: High** *(Feature Orientation)*

- **FR103: Feature Type Taxonomy** — Features are classified into three mutually exclusive types: **flow** (an end-to-end user journey through the system — observable as a complete user action that produces a result), **connection** (an integration or handoff between subsystems or external tools — observable as data or control passing correctly across a boundary), **quality** (a non-functional requirement observable by the user — observable as a system attribute such as speed, reliability, or consistency). All three types appear in feature tables grouped by type in the HTML artifact (FR106). The taxonomy is defined in `features.json` and enforced by `momentum:feature-status` at read time — entries with an unrecognized type are flagged as malformed. **Priority: High** *(Feature Orientation)*

- **FR104: momentum:feature-status Skill** — A standalone skill `momentum:feature-status` reads `features.json` and `_bmad-output/implementation-artifacts/sprints/index.json`, performs a coverage gap analysis for each feature (FR105), and writes a self-contained HTML planning artifact to `.claude/momentum/feature-status.html`. After writing the artifact, the skill opens it in a cmux browser pane. The skill is independently invocable at any point in the lifecycle — it does not require an active sprint. It is also spawned by the retro orchestrator at Phase 6 close (FR108). *Note: this skill supersedes the earlier `/momentum:status` skill (`skills/status/SKILL.md`) — that skill's session-orientation role has been absorbed into Impetus startup-preflight (FR107) and its planning-visualization role is covered by this standalone skill.* **⚠ Deprecated (sprint-2026-05-03): this skill is superseded by `momentum:canvas` (FR125). The delivery mechanism changes — instead of writing a static HTML file opened via `file://`, the canvas skill starts a Hono+Bun server on port 3456 and opens `http://localhost:3456`. The static artifact contract (`feature-status.html`) is retired.** **Priority: High** *(Feature Orientation)*

- **FR105: Coverage Gap Analysis** — For each feature in `features.json`, `momentum:feature-status` must reason about sufficiency: whether the assigned story set plausibly delivers the full `acceptance_condition`. A gap is identified when the stories assigned to a feature do not collectively cover everything required by the acceptance_condition. When a gap is identified, a gap description is produced that names specifically what the acceptance condition requires that the stories do not cover — not a generic "missing coverage" label but a concrete statement of what is absent. Features with identified gaps are marked with a GAP indicator and sorted to the top of their type group in the HTML artifact (FR106). Features with no assigned stories and a non-`working` status are always treated as gapped. *Note (sprint-2026-05-03): two gap-detection mechanisms now coexist. The **LLM-based plausibility check** described above is used in the `momentum:feature-status` skill workflow. The `momentum:canvas` `/lenses/features` route (FR125) uses a **deterministic structural heuristic** instead: `has_gap=true` when `stories_done === 0 AND status !== 'working'`. The canvas route uses this synchronous heuristic because synchronous server routes cannot invoke LLMs.* **Priority: High** *(Feature Orientation)*

- **FR106: HTML Artifact Layout** — The HTML artifact written to `.claude/momentum/feature-status.html` is self-contained (no external dependencies) and contains five sections in order: (1) **Page header** — project name, generated date/time; (2) **Summary stats bar** — total feature count, count per status (working / partial / not-working / not-started), gap count; (3) **Mermaid dependency graph** — collapsed by default behind an HTML `<details>` element, showing feature-to-story dependencies; (4) **Feature tables** — one table per feature type (flow, connection, quality), each table sorted with GAP rows at the top, then by status; each row contains: feature name → status badge (color-coded: green=working, yellow=partial, red=not-working, grey=not-started) → story fraction N/M with an inline progress bar → GAP indicator; the full `acceptance_condition` text and gap description are behind an HTML `<details>` expand on each row and are never visible by default; (5) **Footer** — generated-by line, skill version. The artifact must render correctly in a cmux browser pane without a local web server. *Superseded-partial by momentum-canvas (sprint-2026-05-03): the self-contained static file model is replaced by the live Hono+Bun server (FR125). The layout requirements — type-grouped tables, GAP rows sorted to top, status badges, story fraction, acceptance condition drill-down — migrate to the server-rendered routes `/` and `/lenses/features`. The static `feature-status.html` artifact contract is retired.* **Priority: High** *(Feature Orientation)*

- **FR107: Feature Status Impetus Cache** — After each `momentum:feature-status` run, the skill writes `.claude/momentum/feature-status.md` with YAML frontmatter containing: `input_hash` (SHA of the `features.json` content at generation time), `summary` (one-line prose summary: "N features: X working, Y partial, Z gapped"), and `generated_at` (ISO timestamp). Impetus reads this cache file inline within startup-preflight — no additional Bash subprocess call is needed. Impetus displays one of four feature-status greeting states based on cache state: **no-features** (`_bmad-output/planning-artifacts/features.json` absent — no feature tracking configured), **no-cache** (cache file absent but `features.json` present — feature-status has not been run yet), **fresh** (cache `input_hash` matches current `features.json` SHA — display the cached summary), **stale** (hash mismatch — display the summary with a staleness note). The greeting computation adds zero additional startup tool calls beyond the existing startup-preflight read. *Note (sprint-2026-05-03): following the deprecation of `momentum:feature-status` (FR104), the authorized cache writer changes from `momentum:feature-status` to `momentum:canvas` (FR125). The cache file path (`.claude/momentum/feature-status.md`), YAML frontmatter schema, and Impetus read behavior are unchanged — only the writing skill changes.* **Priority: High** *(Feature Orientation)*

- **FR108: Sprint Boundary Summary** — At retro Phase 6 close (after `momentum-tools sprint retro-complete` is called), the retro orchestrator spawns `momentum:feature-status` to refresh the feature cache, then writes a `sprint-summary.md` file to the sprint's artifact directory (`.momentum/sprints/{sprint-slug}/sprint-summary.md` per the impetus-momentum-state-migration — see FR120). The summary contains five sections: **Features Advanced** (only present when `features.json` exists — lists features whose status changed during this sprint), **Stories Completed vs. Planned** (N of M stories completed with slugs), **Key Decisions** (decisions made during the sprint, drawn from the retro audit), **Unresolved Issues** (open findings or blocked stories carried forward), **Narrative** (1–3 sentence prose summary of the sprint). The full summary is capped at 500 words. Sprint planning Step 1 reads the most recent sprint summary (by sprint slug date order) before synthesizing its 3–5 prioritized recommendations — providing continuity from the prior sprint's findings. **Priority: High** *(Feature Orientation)*

- **FR109: Practice Project Rendering Path** — When `momentum:feature-status` detects a practice project — defined as the presence of a `skills/` directory containing at least one `SKILL.md` file — it renders an additional section in the HTML artifact below the feature tables. This section contains: (1) a **skill topology table** showing each skill's hand-off relationships (which skills it calls and which skills call it), derived from `SKILL.md` frontmatter and workflow references; (2) an **SDLC coverage table** mapping SDLC phases (Intake, Planning, Specification, Implementation, Verification, Retrospective, Distillation) to the skills that cover them — phases with no covering skill are highlighted as gaps. The additional section is under 40 lines of rendered HTML. The product project rendering path (feature tables, stats bar, Mermaid graph) is unaffected — the practice section is additive only. **Priority: Medium** *(Feature Orientation)*

- **FR110: momentum:feature-grooming Skill — Bootstrap and Refine Modes** — Developer can invoke `momentum:feature-grooming`. The skill detects mode automatically based on the current state of `features.json`: **Bootstrap** (`features.json` absent or ≤2 entries) — synthesizes 8–25 candidate features from PRD, epics, architecture, and stories; warns with `!` if the resulting candidate count falls outside the 8–25 range. **Refine** (`features.json` has ≥3 entries) — evaluates the existing feature set and reports six signals: MERGE / SPLIT / DEDUP / NEW / RETIRE / UPDATE; all six signals are reported including zero counts. **Priority: High** *(Feature Orientation)*

- **FR111: feature-grooming Parallel Discovery** — During the discovery phase, the skill spawns exactly 2 discovery subagents in a single message (model: haiku, effort: quick). Agent A reads PRD and epics. Agent B reads architecture and `stories/index.json`. The orchestrator handles all synthesis, value analysis, and developer interaction directly — no additional subagents are spawned beyond these two. **Priority: High** *(Feature Orientation)*

- **FR112: feature-grooming Value Analysis Requirements** — Every feature proposed or evaluated by `momentum:feature-grooming` must include both required fields before being written to `features.json`: `value_analysis` (multi-paragraph) covering (a) current value delivered, (b) full vision including new capabilities beyond pain removal, and (c) known gaps — value analysis must not reduce to pain removal only; and `system_context` (string) describing how the feature fits and enhances the overall product. No feature is written to `features.json` without both fields present and substantive. **Priority: High** *(Feature Orientation)*

- **FR113: feature-grooming Developer Interaction and Write Safety** — The skill presents the full candidate set with value analyses holistically before opening any per-feature gate. Three review questions are asked: (1) feature coverage completeness, (2) accuracy of value analyses, (3) deferred-value confirmation for ⚠ flagged features. Features with no current delivery or entirely deferred value are flagged ⚠; the developer must explicitly confirm inclusion of each such feature. `features.json` is not written until an explicit developer approval gate is passed. In bootstrap mode: an assessment document (aes-NNN) and a decision document (dec-NNN) are written to `planning-artifacts/assessments/` and `decisions/` respectively before `features.json` is written. After writing `features.json`, the skill calls `momentum-tools feature-status-hash` and reports the resulting hash and the count of unmapped stories (non-dropped, non-done stories not assigned to any feature). Commit message: `docs(features): feature-grooming {bootstrap|refine} — N features, M proposals applied`. **Priority: High** *(Feature Orientation)*

- **FR118: momentum:feature-breakdown Skill** — Developer can invoke `momentum:feature-breakdown` with a required `feature_slug` (must be a key in `features.json`) and an optional focus hint. The skill enumerates the story gaps required to ship the feature end to end, then delegates the resulting gap list to `momentum:triage` for classification and routing. Internal method: the skill loads the target feature's context plus surrounding product context, then spawns exactly 2 gap-analysis subagents in a single message (fan-out — Agent A: Acceptance-first, Agent B: Value-gap-first); the orchestrator synthesizes their outputs into a deduplicated gap list, presents it to the developer for review and approval, then calls `momentum:triage` with `source_label = "feature-breakdown:{feature_slug}"` so that any ARTIFACT items written to `intake-queue.jsonl` carry the correct upstream emitter. Hard dependency: `momentum:triage` — the skill fails fast if triage is unavailable. Role: pure orchestrator — the skill NEVER writes to `features.json`, `stories/index.json`, or any other planning artifact directly. Boundary: `feature-breakdown` identifies candidates; triage and the developer decide what becomes a story. **Priority: High** *(Feature Orientation)*

- **FR125: momentum:canvas Skill** — Developer can invoke `/momentum:canvas` to open the Momentum Cycle live dashboard. The skill checks if port 3456 is listening; if not, it starts a Hono+Bun server (`bun --hot server.tsx`) in the cmux services pane via `cmux respawn-pane`; then opens `http://localhost:3456` in the cmux viewer pane. The dashboard persists across sessions — subsequent invocations skip the server start if port 3456 is already listening. Bun must be available on the developer's PATH. This skill supersedes the `momentum:feature-status` static-file approach (FR104). Authorized writer of the Impetus feature-status cache (FR107). **Priority: High** *(Feature Orientation — Canvas)*

- **FR126: Live HTMX Dashboard Polling** — The Momentum Cycle dashboard live-polls lens data from the Hono server. Features lens: 2-second polling interval. Cycle timeline lens: 5-second polling interval. Sprint lens: 2-second polling interval. Polling updates the lens container `innerHTML` without a page reload, providing always-current state without manual re-invocation. **Priority: High** *(Feature Orientation — Canvas)*

- **FR127: Sprint Lens** — The Momentum Cycle dashboard Sprint lens shows the active sprint identifier, start date, and retro completion state (green "Retro done" / amber "Retro pending" based on `retro_run_at`). When no active sprint exists, an empty-state message is shown. Clicking the sprint card navigates to a sprint detail view showing stories grouped into Blocked, In Progress, and Validated outcome bands. **Priority: High** *(Feature Orientation — Canvas)*

- **FR128: Cycle Timeline Lens** — The Momentum Cycle dashboard Cycle lens shows a 7-node horizontal timeline of the current cycle's phase progression (triage, feature-grooming, epic-grooming, refine, sprint-planning, sprint-dev, retro). Nodes display one of four states: **done** (filled indigo), **next-required** (indigo ring with glow — required phases only: sprint-planning, sprint-dev, retro), **not-run** (gray, optional phases skipped), **pending** (gray, not yet reached). Cycle boundary is derived from the most recent `retro_run_at` timestamp in `sprints/index.json`. **Priority: High** *(Feature Orientation — Canvas)*

- **FR129: Feature L2 Drill-Down** — The Momentum Cycle dashboard Feature lens rows navigate to a reading-mode detail view on click. The detail view renders six sections (heading, meta strip, value narrative, acceptance condition, system context, stories list) in Source Serif 4 at 65ch / 18px / 1.70 line-height. Page polarity flips from dark dashboard to warm light (`--readingPaper: #faf6ec`) via a 140ms CSS cross-fade. Navigation is via HTMX `hx-push-url` with breadcrumb OOB swap. **Priority: High** *(Feature Orientation — Canvas)*

- **FR130: Story L3 Drill-Down** — The Momentum Cycle dashboard Story detail view renders individual story files (`.momentum/stories/{slug}.md`) as a typographic reading document. Sections: frontmatter strip (JetBrains Mono), title (Source Serif 4), value narrative, numbered acceptance criteria, dev notes (collapsible via `<details>`/`<summary>` when more than 3 items), touches list. Entry from Feature L2 shows "Dashboard / Feature / Story" breadcrumb; entry from Sprint detail shows "Dashboard / Sprint / Story" breadcrumb. Always warm-light reading mode. **Priority: High** *(Feature Orientation — Canvas)*

### Triage and Unified Intake Queue

- **FR114: momentum:triage Multi-Item Batch Classification Skill** — Developer can invoke `/momentum:triage` to process N observations (surfaced during the current session or accumulated from prior sessions) in a single skill run. The skill classifies each observation into one of six classes — **ARTIFACT** (promote to story via `momentum:intake`), **DISTILL** (apply a practice-artifact change via `momentum:distill`), **DECISION** (capture a strategic choice via `momentum:decision`), **SHAPING** (record as an open item needing further shaping), **DEFER** (record as a watch item revisited later), **REJECT** (record as explicitly not-pursued with rationale). ARTIFACT items are enriched with `feature_slug` (DEC-005 D1), `story_type` (DEC-005 D5), `epic`, `priority`, and `dependencies` before being handed to `momentum:intake`. The skill uses a batch-approval UX: the developer confirms the full classification plan before any write occurs. Delegation: ARTIFACT → `momentum:intake`; DISTILL → `momentum:distill`; DECISION → `momentum:decision`; SHAPING / DEFER / REJECT → written to `intake-queue.jsonl` (FR115) via the momentum-tools CLI. On session start, the skill reads open `kind: "shape"` and `kind: "watch"` items (status `open`) from the queue and re-surfaces them alongside new observations so nothing is lost between sessions. This requirement partially supersedes the earlier raw-input → `epics.md` triage contract described in FR49. **Priority: High**

- **FR115: Unified intake-queue.jsonl Event Log** — The practice maintains a single append-only JSONL event log at `.momentum/intake-queue.jsonl` (per the impetus-momentum-state-migration — see FR120) that captures all non-story triage outcomes and cross-skill handoff items. Events carry two discriminators: **`source`** (one of `triage` | `retro` | `assessment` | `feature-breakdown` — reflects the ORIGINAL upstream emitter, not just the proximate writer; when triage routes items originally enumerated by `momentum:feature-breakdown`, the `source` value is `feature-breakdown` and the `source_label` field carries `feature-breakdown:{feature_slug}`) and **`kind`** (one of `shape` | `watch` | `rejected` | `handoff`). Base schema fields on every event: `id` (ULID or timestamped slug), `source`, `kind`, `title` (short human-readable), `description` (one-to-three-sentence summary), `status` (one of `open` | `consumed` | `rejected`), `created_at` (ISO-8601 UTC). Optional fields (present when applicable): `sprint_slug` (provenance when emitted by a sprint-bound skill), `feature_slug`, `story_slug`, `resolved_at` (when status transitions off `open`), and retro-specific optional enrichment fields (`feature_state_transition`, `failure_diagnosis`, `suggested_feature_slug`, `suggested_story_type`, `evidence_refs` — see FR116). This artifact is the single source of truth for non-story triage outcomes and retro → planning handoff. It supersedes the never-built `intake-queue.jsonl` master-plan contract and the never-built `triage-inbox.md` architecture.md contract per **DEC-007**. All writes flow through the momentum-tools CLI — skills do not write JSONL directly (orchestrator purity). Authorized writers: `momentum:triage` (shape / watch / rejected), `momentum:retro` (handoff), future `momentum:assessment` extensions. **Priority: High**

- **FR116: Retro → intake-queue Handoff Producer** — `momentum:retro` Phase 5 (action-item triage) gains a third disposition — "carry forward as handoff event" — alongside the existing Tier 1 distill path and Tier 2 stub path. Any action item the developer does not immediately distill or stub is recorded (with developer approval — no silent emission) as a JSONL event appended to `intake-queue.jsonl` with `source: "retro"`, `kind: "handoff"`, `status: "open"`, via the momentum-tools queue-write CLI (FR115). Handoff events carry optional enrichment fields when applicable: **`feature_state_transition`** `{feature_slug, prior_state, observed_state, evidence}` (DEC-005 D8 — retro as feature-state hygienist), **`failure_diagnosis`** `{attempted, didn_t_work, learned}` (DEC-005 D7 — failure as legitimate diagnostic category), **`suggested_feature_slug`** (DEC-005 D1), **`suggested_story_type`** (DEC-005 D5 — one of `feature` | `maintenance` | `defect` | `exploration` | `practice`), **`evidence_refs`** (pointers back to the findings document). Per DEC-005 D10, retro performs **no value-floor gap-check** when emitting handoff events — it records context; downstream consumers (triage at session start, sprint-planning at backlog synthesis) decide. Developer-approval gate is preserved: Phase 5 prompts Y/N for each handoff carry-forward using the same pattern as stub creation. **Priority: Medium**

- **FR117: Sprint-Planning Handoff Queue Read** — `momentum:sprint-planning` Step 1 Phase A.5 is extended to read `.momentum/intake-queue.jsonl` (per the impetus-momentum-state-migration — see FR120) filtered to `source: "retro"`, `kind: "handoff"`, `status: "open"` immediately after the previous sprint summary load (FR108). Open handoff entries are folded into the synthesis context that feeds Phase C recommendation output. Phase C output gains an additional labeled section — **"Open handoff items from recent retros"** — that lists each open handoff entry by title, source sprint, and (when present) its feature-state transition or failure-diagnosis framing. The developer sees retro-surfaced handoff context as part of backlog synthesis without having to re-inject findings verbally. The read is defensive: if `intake-queue.jsonl` does not exist (first-ever sprint after FR115 ships), the read returns empty and planning proceeds with only the previous sprint summary. **Priority: Medium**
...
- **NFR20: Startup Performance** — The session greeting (FR54) must render near-instantly. Impetus reads `stories/index.json` and `sprints/index.json`, determines the greeting state, and renders the narrative greeting without delays. No multi-minute startup sequences, no sequential file scans, no heavy computation before the greeting appears. Validated by: greeting appears within the first tool-use cycle of `/momentum:impetus` invocation. *[Extended by NFR21 — feature-status cache check adds zero additional Bash calls to the startup sequence.]*

- **NFR21: Startup Bash Call Ceiling** — The Impetus greeting computation must not add any Bash subprocess calls beyond the single startup-preflight call already required by NFR20. The feature-status cache check (FR107) is performed by reading `.claude/momentum/feature-status.md` inline within startup-preflight — it does not issue a separate Bash call. The happy path from `/momentum:impetus` invocation to greeting display remains exactly one Bash call. Any new greeting-state data added to startup-preflight must be read from files already fetched in that single call, not from additional subprocess invocations. Validated by: trace of a cold-start `/momentum:impetus` session shows exactly one Bash tool-use call before greeting output appears. *[Extended 2026-04-28: the plugin-cache staleness check (FR119) — added by sprint-2026-04-27 — preserves this latency ceiling. The `momentum-tools session plugin-cache-check` subcommand absorbs into the single startup-preflight call (no additional Bash subprocess calls). The cache-vs-source version comparison is performed within the existing preflight invocation and surfaces its result alongside the other preflight outputs read inline by Impetus.]*
...
- **FR55:** All writes to `stories/index.json` and `sprints/index.json` (under `.momentum/...` per the impetus-momentum-state-migration — see FR120) go through `momentum-tools.py`, a Python CLI tool invoked via Bash. No other agent, skill, or script writes to these files directly. This ensures atomic status transitions and prevents concurrent write conflicts. **Priority: High** *(Redesign Foundation)*
...
- **FR120: `.momentum/signals/` Ledger Directory** — The practice maintains a read-only pending-work-flags ledger at `.momentum/signals/` (alongside `.momentum/sprints/`, `.momentum/stories/`, `.momentum/intake-queue.jsonl` per the impetus-momentum-state-migration). The ledger holds discrete signal entries each carrying: `signal_type` (string — see recognized types below), `origin` (the skill/source that wrote the signal — e.g., `momentum:retro`, `momentum:triage`, `momentum:avfl`), `created` (ISO-8601 UTC timestamp), `payload` (signal-type-specific structured data), and an optional `cleared` field (ISO timestamp populated when the signal is resolved). Initial recognized `signal_type` values: **`triage-uncleared`** (open shape/defer/handoff items in `intake-queue.jsonl` that need re-surfacing on session start) and **`avfl-finding-pending-upstream-fix`** (an AVFL critical finding awaits an upstream-level remediation that has not been queued or applied). Intended writers: `momentum:retro`, `momentum:triage`, `momentum:avfl`. Intended reader: Impetus session orientation (read-only — Impetus does not clear signals; the originating skill is responsible for clearing). An empty `.momentum/signals/` directory is valid (no pending signals). This requirement closes **DEC-011 Gate G1** for the canvas redesign — the canvas planning artifact may now assume a stable structured signals ledger as input. **Priority: High** *(Sprint State Schema)*

### Agent Observability

- **FR56:** REMOVED — Agent journal write infrastructure deleted (sprint-2026-04-08). DuckDB transcript audit (Decision 27) is the sole evidence source for retrospectives. Agent JSONL logs via `momentum-tools log` are no longer written. **Priority: High** *(Phase 3)*
- **FR57:** REMOVED — Agent fault-tolerant logging requirement deleted (sprint-2026-04-08). Same rationale as FR56 — journal write infrastructure removed, DuckDB transcript audit is the sole evidence source. **Priority: High** *(Phase 3)*
- **FR85:** When the sprint-dev spawn registry (FR84) suppresses a duplicate agent spawn, the suppression is observable via DuckDB transcript audit — the spawn registry's decision to suppress is captured in the transcript record and queryable via `transcript-query.py` (FR87). No separate logging mechanism is required; the transcript is the evidence source. The spawn registry (FR84) still exists and enforces deduplication — only the observability mechanism changes from explicit `momentum-tools log` calls to transcript-based audit. **Priority: High** *(Phase 3)* *[No backlog story exists]*
- **FR89:** REMOVED — SubagentStart/SubagentStop hooks deleted (sprint-2026-04-08). Subagent lifecycle is observable via DuckDB transcript audit; dedicated hook logging is not required. **Priority: High** *(Phase 3)*

### Gherkin Separation

- **FR58:** Story markdown files contain plain English acceptance criteria only. Sprint-planning (Step 3.5) authors one verification contract per story in `.momentum/sprints/{sprint-slug}/specs/`; contract format (`.eval.yaml`, `.trigger.md`, `.smoke.sh`, `.review.md`, `.feature`) is determined by the story's highest-tier change-type (per the routing table in `verification-standard.md`). After contract authoring, a spec quality pre-check validates structure, outsider-test compliance, and template conformance before dev agents spawn — catching spec defects before they propagate to implementation. Dev agents never access the specs directory — the read-bar applies to all contract file types. Verifier agents read contracts exclusively from this path. This enforces black-box behavioral validation — developers implement against intent (plain English ACs), while verifiers validate against precise behavioral specifications (contract format by change type). **Priority: High** *(Phase 3)*

### Sprint Planning & Execution

- **FR59:** Sprint planning workflow begins by reading the master plan (`epics.md` or equivalent) as Step 1 — establishing full project context before any story-level work. Step 1 includes a staleness check via `git log` on the master plan file to surface how recently it was updated and whether it may be out of date. The workflow then leads with 3-5 prioritized recommendations for the sprint based on epic priorities, dependency readiness, and backlog state — the developer reviews these before proceeding to detailed selection. Subsequent steps include: backlog presentation (grouped by epic, excluding terminal states), story selection (3-8 stories with dependency warnings), story fleshing-out (spawn `momentum:create-story` for stubs), **per-story approval gate (FR121)** — at Step 3 each fleshed story is opened in a cmux markdown viewer pane and the developer is presented a three-option prompt (Approve / Revise / Reject); approvals are persisted as records in `sprints.planning.approvals[]` with `story_slug`, `approved_at`, `decision`, and `story_file_sha` — Gherkin spec generation, team composition, AVFL validation of the complete plan, developer review, and sprint activation. Once `momentum-tools sprint activate` is called, the sprint is immutable — no in-place patching of story selection, team composition, or dependency graph. If a sprint must change after activation, the recovery path is: close the sprint via `momentum-tools sprint close` (status `closed-incomplete`), migrate incomplete stories to the next sprint's backlog, and plan a new sprint. Planning decisions are logged throughout via the agent logging tool. **Priority: High** *(Phase 3)*
- **FR134: Frozen Verification Contracts and Holistic Coverage Plan** — Sprint-planning Step 3.5 executes three phases: (a) Contract authoring — one contract per story in `.momentum/sprints/{sprint-slug}/specs/`, format determined by the story's highest-tier change-type, body states only observable behaviors. (b) Coverage plan — `coverage-plan.md` at sprint root, maps minimal integrated scenarios to stories they discharge, marks 'covered-by-composition' vs 'dedicated-run', states anti-redundancy principle. (c) Adversarial guard — a decorrelated adversarial agent (not the contract author) applies the Outsider Test clause-by-clause; failing clauses are rewritten and re-checked up to 2 passes; residual failures are surfaced to the developer before activation. Step 3.5 runs between per-story approval (FR121) and Gherkin spec generation; Step 4 does not overwrite existing `.feature` contracts. **Priority: High** *(Sprint Planning & Execution)*
- **FR135: Sprint Activation Contract Gate and Post-Activation Immutability** — `momentum-tools sprint activate` fails if any story in the sprint lacks a verification contract in `.momentum/sprints/{sprint-slug}/specs/` or if `coverage-plan.md` is absent from the sprint root. After activation, `specs/**` and `coverage-plan.md` are hook-immutable — a PreToolUse hook blocks Write, Edit, and Delete operations on those paths. Dev agent Read access to `specs/` is also blocked post-activation. **Priority: High** *(Sprint Planning & Execution)*
...
- **FR95: Quality Gate Parity** — All execution paths (`momentum:sprint-dev`, `momentum:quick-fix`) must apply equivalent quality gates: pre-implementation AVFL checkpoint, post-merge AVFL scan, code review via `momentum:code-reviewer`, and team validation. `momentum:dev` is a pure internal executor — not user-invocable. Single-story development uses `momentum:quick-fix`. **Priority: High** *(Phase 3)*
...
- **FR121: Per-Story Approval Gate** — Sprint planning Step 3 introduces a per-story approval gate. After each story is fleshed out (via `momentum:create-story` for stubs, or in-place for already-fleshed stories), the orchestrator opens the resulting story file in a cmux markdown viewer pane and prompts the developer with a three-option choice: **Approve** (story is ready for the sprint), **Revise** (return to fleshing — the orchestrator iterates until the developer is satisfied), **Reject** (drop the story from the sprint and re-evaluate selection). Approval records are persisted in the sprint record under `sprints.planning.approvals[]` — each entry contains: `story_slug`, `approved_at` (ISO-8601 UTC), `decision` (`approved` | `revised` | `rejected`), and `story_file_sha` (SHA of the story file at approval time, used to detect drift between approval and activation). Two new `momentum-tools sprint` subcommands implement the gate: **`story-approve`** writes an approval record; **`verify-approvals`** is invoked at the activation precondition gate (FR62) and asserts every story in the sprint has an `approved` decision with a `story_file_sha` that matches the current story file SHA. Activation is blocked unless `verify-approvals` passes. Approvals carry through the sprint lifecycle — they remain in the sprint record across `planning → active → completed`, providing an audit trail of what was approved and when. **Priority: High** *(Sprint Planning)*

### Sprint Retrospective

- **FR66:** The retrospective workflow's primary data source is a DuckDB transcript audit, not milestone logs. The `transcript-query.py` tool (FR87) ingests Claude Code session transcripts (JSONL) into DuckDB for structured analysis. DuckDB transcript audit is the **sole** evidence source per ARCH-5 / Decision 27 — there is no secondary evidence source. A 4-agent auditor team performs the retrospective with an explicit team-composition topology: the **documenter** is declared as a singleton coordinator (Shape A: individual `Agent` spawn, or single-cardinality `TeamCreate` role) distinct from the auditor fan-out group; **three auditors** — `auditor-human` (developer-experience patterns), `auditor-execution` (transcript / tool-use / agent lifecycle), `auditor-review` (project quality, architectural drift, spec gaps) — fan out as 3 individual Agent spawns each carrying the documenter handle. This topology closes the RV-05 / E14 / E15 documenter-replication defect (FR124). Retro Phase 4 enforces a post-`TeamCreate` singleton-count guard (FR123) that asserts exactly 1 documenter + 1 each of `auditor-human` / `auditor-execution` / `auditor-review` (4 total, no duplicates, no surplus); on failure the workflow HALTs with diagnostic — there is no continue-anyway path. Retro Phase 2 hard-fails on zero-session extraction — if `transcript-query.py` returns zero sessions for the sprint date window, the workflow stops with a diagnostic (transcript path resolution, story-slug filter, date window) and does NOT proceed with empty extracts; there is no continue-with-empty-extracts path. The auditor team produces two triage outputs synthesized via the documenter: Momentum triage (practice-level issues feeding back into Momentum's refinement cycle) and Project triage (project-level issues feeding back into the project's refinement cycle). Retro can transition unfinished stories to `closed-incomplete` status. Actionable findings from either triage output are converted into story stubs for the next sprint's backlog. On completion, retro calls `momentum-tools sprint retro-complete` for final sprint closure. **Priority: High** *(Phase 5)*
- **FR87: Transcript Query Tool** — `transcript-query.py` is a DuckDB wrapper tool that ingests Claude Code session transcripts (JSONL format) into an in-memory DuckDB database for structured SQL analysis. The tool provides: (1) pre-built queries for common retro patterns (error frequency by agent, tool-use heatmaps, phase duration analysis, spawn/completion timelines, retry patterns); (2) ad-hoc SQL capability for exploratory analysis; (3) session auto-discovery by sprint date range — the tool scans transcript directories, matches sessions to the sprint's active date window, and loads all matching transcripts without manual file specification. Output is tabular (stdout) or JSON (for programmatic consumption by auditor agents).

  **Hardening (sprint-2026-04-27):** `transcript-query.py` is hardened on six dimensions: (1) **Worktree path discovery** — the tool resolves all worktree paths via `git worktree list --porcelain`, ensuring transcripts written to worktree-scoped session directories (sprint-dev / quick-fix execution) are discoverable in addition to main-repo session directories. (2) **UTC end-of-day inclusivity at `--before`** — the `--before` date filter is interpreted as inclusive end-of-day in UTC, so a sprint window ending on YYYY-MM-DD includes all sessions through 23:59:59.999 UTC of that date. (3) **`json.dumps`-only serialization** — all JSON output flows through `json.dumps` (no string concatenation or manual escape), eliminating malformed-JSON failure modes for nested structures. (4) **Dynamic script-path resolution** — the tool's location is resolved dynamically from `~/.claude/plugins/cache/momentum/momentum/*/scripts/transcript-query.py` rather than a hardcoded path, surviving plugin version updates. (5) **`--story-slugs` filter for same-day sprints** — when multiple sprints share a date (e.g., `sprint-2026-04-03` and `sprint-2026-04-03-2`), the `--story-slugs` filter scopes session matching to only sessions whose story-slug attribution matches the supplied list, preventing cross-sprint transcript contamination. (6) **Peek-first large-file convention** — the convention of peeking file sizes before full reads is propagated to retro auditor / documenter prompts and to sprint-planning Step 4.5 spec-impact-discovery agents, so transcript-derived large artifacts are read in chunks rather than risking context blow-out. **Priority: High** *(Phase 5)*
- **FR91: Transcript Error Detection Standards** — `transcript-query.py` error detection must use structural error indicators (event types, exit codes, tool failure signals, agent lifecycle events) rather than string matching on message content. String matching on error-like words in conversational text produces unacceptable false-positive rates. The false-positive rate for error detection queries must be below 5% as measured against a manually labeled sample of at least 50 transcript sessions.

  **Accuracy-of-input dependencies (sprint-2026-04-27):** Structural error detection accuracy is bounded by transcript-input completeness. Two FR87 hardening dimensions are explicit accuracy callouts for FR91: (1) **Path resolution accuracy** — worktree-scoped sessions discovered via `git worktree list --porcelain` (FR87 hardening dim 1) must be included; transcripts missed because they live in worktree session directories will silently understate error counts. (2) **Slug-filter accuracy for same-day sprints** — the `--story-slugs` filter (FR87 hardening dim 5) must scope sessions to the correct sprint when multiple sprints share a date; cross-sprint transcript contamination corrupts both true-positive and false-positive measurement. Error-detection accuracy claims are valid only when both inputs are correctly resolved. **Priority: High** *(Phase 5)* *[`transcript-query-calibration` story is done and covers the structural-indicator-not-string-matching requirement. The quantitative <5% FP rate validation standard (measuring against ≥50 hand-labeled sessions) is **deferred** as of 2026-04-26 — no backlog story exists to implement the labeling exercise, and DuckDB-based structural detection has been adequate in practice. Re-open this requirement only if retro audits begin surfacing FP-rate concerns. The structural-detection portion of FR91 stands as implemented; the empirical FP-rate validation portion is explicitly deferred without a covering story.]*
- **FR123: Retro Team Singleton-Count Guard** — Retro Phase 4 enforces a post-`TeamCreate` singleton-count guard. Immediately after the auditor team is created, the guard reads the team configuration and asserts: **exactly 1 documenter** + **exactly 1 each of `auditor-human`, `auditor-execution`, `auditor-review`** (4 agents total — no duplicates, no surplus, no missing roles). On any failure (duplicate role, surplus agent, missing role, or other deviation), the guard HALTs the workflow with a structured diagnostic identifying the deviation. There is **no continue-anyway path** — the workflow cannot proceed past Phase 4 without a passing guard verdict. This guard codifies the topology established by FR124 and prevents silent regressions where future workflow edits accidentally re-introduce documenter replication or auditor cardinality drift. **Priority: High** *(Sprint Retrospective)*

- **FR124: Retro Documenter-Auditor Topology — Documenter as Singleton Coordinator** — Retro Phase 4 team-composition topology is fixed: the **documenter** is declared as a singleton coordinator (Shape A: an individual `Agent` spawn, OR a single-cardinality `TeamCreate` role) — distinct from the auditor fan-out group. The **three auditors** (`auditor-human`, `auditor-execution`, `auditor-review`) are declared as a separate fan-out group, each spawned as an individual `Agent` carrying the documenter handle for synthesis hand-off. This topology closes the **RV-05 / E14 / E15** documenter-replication defect — an earlier topology declared the documenter as a TeamCreate role, which silently spawned three documenter instances (one per auditor) and produced inconsistent synthesis. The fix is structural: documenter and auditor groups are declared on different topology axes (singleton coordinator vs. fan-out group) so cardinality is unambiguous at workflow-definition time. FR123's runtime guard provides a second defense layer. **Priority: High** *(Sprint Retrospective)*

- **FR96: Retro Signal Classification** — Each finding in the retro audit carries two orthogonal classification dimensions: (1) **Behavioral** (existing): `correction | redirection | frustration | praise | decision` — captures what the developer did; (2) **Causal** (Fowler signal taxonomy): `Context | Instruction | Workflow | Failure` — captures what category of practice artifact needs updating. The causal dimension is stored as `signal_type` in the findings-ledger schema and determines routing: Context → CLAUDE.md / priming docs; Instruction → skill references / shared prompt templates; Workflow → workflow step definitions; Failure → anti-pattern rules / guardrails. The documenter agent applies both dimensions to each finding at write time. The two dimensions are independent — a finding can be behavioral type `correction` and causal type `Instruction` simultaneously. In retro Phase 5, Tier 1 findings (small, immediately applicable) are routed to `/momentum:distill` instead of creating story stubs; Tier 2 findings (structural changes) continue through Phase 5's existing stub creation path. **Priority: High** *(Practice Compounds)*
...
Epics remain the primary unit for grouping related stories and managing scope. Sprints are the primary unit for execution — stories selected into a sprint may come from multiple epics. `momentum:create-story` adds stories individually; sprint planning selects stories for execution; `momentum:sprint-dev` provides the cross-epic execution model.
...
- **FR68:** The sprint record in `.momentum/sprints/index.json` (per the impetus-momentum-state-migration — see FR120) stores team composition (roles, guidelines, story assignments) and the dependency graph. Sprint planning writes this record; `momentum:sprint-dev` reads it. The schema is defined by the architecture document. **Priority: High** *(Phase 3)*
...
- **FR74: Hybrid Resolution Team** — After per-story code review (FR88) and post-merge AVFL scan (FR73), `momentum:sprint-dev` spawns a concurrent Agent Team to resolve consolidated findings from both sources: Dev fixes findings, QA validates story ACs against acceptance criteria, E2E Validator tests running behavior against verification contracts using external tools, Architect Guard checks pattern drift against the architecture document. The team operates concurrently on the main branch. Each agent role receives project-specific guidelines from the sprint record (FR61).

  **E2E Validator method-polymorphic contract (sprint-2026-05-17):** The E2E Validator is method-polymorphic — it accepts contracts in any format (`.eval.yaml` EDD eval, `.trigger.md` behavioral trigger, `.smoke.sh` shell execution, `.review.md` document review, `.feature` Gherkin) and dispatches the appropriate execution strategy by file extension. Environment startup, driver binding, human-review carveouts, and trivial-smoke escape are resolved from `momentum/verification-harness.json` at startup. If `verification-harness.json` is absent, the agent reports BLOCKED. No stack-specific references appear in the agent body. **Priority: High** *(Phase 3)*

  **QA Reviewer parallel contract (sprint-2026-04-27):** The QA Reviewer agent definition is held to the same contract as the E2E Validator (parallel, not weaker): (1) **Live-test execution** — must follow project `.claude/rules/e2e-validation.md` Environment Startup; (2) **Spawn-prompt service-state context is context, not permission to skip** — any service-state information conveyed by the orchestrator at spawn time is informational; the QA Reviewer must still verify state independently and execute live tests rather than treating pre-announced "service is up" framing as a substitute for execution; (3) **MISSING vs BLOCKED disambiguation** — when infrastructure required for live test execution is absent, the QA Reviewer reports BLOCKED (not MISSING) — MISSING means the AC has no implementation to test; BLOCKED means the AC has implementation that could not be exercised due to environment. The QA Reviewer agent definition gains an Environment Prerequisites section + a Critical Constraints section parallel to the E2E Validator's, so the agent-definition layer is self-sufficient against spawn-prompt regression. **Priority: High** *(Phase 3)*

- **FR122: Harden Sprint-Dev Phase 5 Spawn Prompts** — Defense-in-depth at the sprint-dev Phase 5 reviewer spawn block. Two prohibitions are made explicit at the spawn-prompt layer: (1) **No service-state pre-announcement** — the orchestrator must not include "service is up" / "stack is running" / "environment is ready" framing in the QA Reviewer or E2E Validator spawn prompt; service state is for the agent to verify, not for the orchestrator to assert. Pre-announcement creates a trust loop where the agent assumes execution is unnecessary because the orchestrator already certified it. (2) **No pytest substitute for live testing** — the orchestrator must not offer pytest (or any unit-test runner) as an acceptable substitute for live behavioral testing of acceptance criteria. Unit tests verify code units; AC verification requires exercising the running system. The QA Reviewer agent definition (FR74) is the second defense layer — it gains an **Environment Prerequisites** section and a **Critical Constraints** section parallel to the E2E Validator's existing constraints, enumerating the same three obligations: live-test execution, AC-evidence-against-execution (each AC verdict carries evidence of a live exercise), and MISSING-vs-BLOCKED disambiguation. The agent-definition layer must be self-sufficient against spawn-prompt regression — even if a future spawn prompt drifts and includes prohibited framing, the agent's own constraints reject the framing and proceed with independent verification. **Priority: High** *(Quality Gates — Phase 5)*
- **FR88: Per-Story Adversarial Code Review** — After each story merges to main, `momentum:sprint-dev` spawns an adversarial code review (via `momentum:code-reviewer`) scoped to that story's changeset. The review runs independently per story — it does not wait for all stories to merge. Findings are accumulated and consolidated with AVFL scan findings (FR64) into a single fix queue before Agent Team handoff (FR74). Per-story review catches story-scoped issues (logic errors, missed ACs, convention violations) that may be obscured by cross-story AVFL analysis. **Priority: High** *(Phase 3)* *[Covering story dropped — no replacement: `code-reviewer-skill-performs-adversarial-review` was dropped from the backlog. The sprint-dev integration requirement (post-merge per-story adversarial review) is currently unimplemented with no replacement story — this remains an active gap.]*
...
- **FR97: Practice Artifact Distillation** — Developer can invoke `/momentum:distill` to capture a session learning or retro finding and immediately apply it to the appropriate practice artifact, without sprint activation or backlog management. The skill is the practice-artifact analogue of `/momentum:quick-fix` (which targets code stories). Invokable at any point during a session or from retro Phase 5 for Tier 1 findings.

  **Discovery phase (parallel, 2 agents):** Enumerator agent reads relevant spec/skill files — maps what exists, where the proposed change fits, what's similar, how it integrates. Adversary agent challenges the proposed change — is it redundant, conflicting, or the wrong abstraction level? Orchestrator synthesizes both into a change plan before writing anything.

  **Fix scope classification — three paths determined during discovery:**
  - **Project-local:** The learning applies only to the current project's rules/references. Distill applies it directly, commits.
  - **Momentum-level, in Momentum project:** The learning is a change to Momentum's own practice files. Distill applies it, bumps the plugin patch version (`version-on-release.md` convention), commits, and pushes. Full capture-and-release in one session.
  - **Momentum-level, in external project:** Developer chooses: (1) Defer to retro queue, or (2) Generate a remote distill prompt — a pre-filled prompt the developer pastes into a Momentum session to apply the fix there.

  **Two-tier model:**
  - Tier 1 (immediate): Small rule additions, anti-pattern notes, reference updates — applies directly, commits.
  - Tier 2 (structural): Changes to workflow steps or spec documents — creates a story stub.

  **Validation:** After changes are written, distill runs a scoped AVFL pass using the `distill` profile (Decision 42 in architecture) on only the changed files — two subagents (Enumerator + Adversary), single-pass, no multi-lens parallelism, Sonnet models at medium-low effort.

  **Audit trail:** Distill writes a record to the findings-ledger with `origin: distill` (Decision 1c extended — distill is an authorized ledger writer alongside the flywheel workflow), enabling FR33 ratio tracking across both origin types.

  **Retro integration:** Retro Phase 5 invokes distill for Tier 1 signal-classified findings instead of creating story stubs. Tier 2 findings continue through Phase 5's existing stub creation path. **Priority: High** *(Practice Compounds)*

- **FR98: Guided Product State Evaluation** — Developer can invoke `/momentum:assessment` to capture a point-in-time evaluation of product state, producing an ASR (Assessment State Record) document at `planning-artifacts/assessments/`. ASRs capture current state, gaps, risks, and recommended next steps. The skill is invokable at any point — between sprints, mid-sprint when a strategic question surfaces, or at the start of a new planning cycle. Output is a structured document with provenance annotations. **Priority: High**

- **FR99: Strategic Decision Capture from Assessments** — Developer can invoke `/momentum:decision` to formalize a strategic choice identified during an assessment or conversation, producing an SDR (Strategic Decision Record) at `planning-artifacts/decisions/`. SDRs include rationale, tradeoffs, phased implementation plan, and decision gates. The skill is typically invoked after `/momentum:assessment` surfaces a recommended next step, but is independently invokable for any strategic decision. **Priority: High**

- **FR100: Lightweight Story Capture from Conversation** — Developer can invoke `/momentum:intake` to capture a story idea discussed in the current conversation, producing a story stub in `stories/index.json` without interrupting the current workflow. The skill is designed to be fast — it captures the idea immediately so nothing is lost — and leaves detailed spec authoring to a later `momentum:create-story` or sprint planning invocation. In addition to the existing epic assignment flow, intake also captures `feature_slug` (DEC-005 D1 — feature-first practice) via a `features.json`-backed suggestion flow analogous to the existing epic flow, and `story_type` (DEC-005 D5 — one of: `feature` | `maintenance` | `defect` | `exploration` | `practice`). The stub template and the `momentum-tools sprint story-add` CLI are extended with `--feature-slug` and `--story-type` flags to accept these fields. **Priority: High**

- **FR101: Plugin Developer E2E Validation Workflow** — Developer can run an E2E validation workflow that exercises the full plugin lifecycle via CMUX — install, upgrade, skill invoke, hook fire — before release. This workflow provides a repeatable pre-release verification path that confirms the plugin installs cleanly, skills are addressable via their namespaced invocations, hooks fire correctly on trigger events, and an upgrade from the previous version applies without error. **Priority: High**
