...
- **FR117: Sprint-Planning Handoff Queue Read** — `momentum:sprint-planning` Step 1 Phase A.5 is extended to read `intake-queue.jsonl` filtered to `source: "retro"`, `kind: "handoff"`, `status: "open"` immediately after the previous sprint summary load (FR108). Open handoff entries are folded into the synthesis context that feeds Phase C recommendation output. Phase C output gains an additional labeled section — **"Open handoff items from recent retros"** — that lists each open handoff entry by title, source sprint, and (when present) its feature-state transition or failure-diagnosis framing. The developer sees retro-surfaced handoff context as part of backlog synthesis without having to re-inject findings verbally. The read is defensive: if `intake-queue.jsonl` does not exist (first-ever sprint after FR115 ships), the read returns empty and planning proceeds with only the previous sprint summary. **Priority: Medium**
