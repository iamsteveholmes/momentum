...
13. **Every `agent()` call pins an explicit model.** All `agent()` call sites in the audit script pass an explicit `model:` override (default `'sonnet'` — retro-grade analysis runs on Sonnet per developer directive). Unpinned calls inherit the session model, which fails under [1m]-context sessions ("Usage credits required for 1M context" entitlement error — observed live 2026-07-06: the sprint-2026-06-28 retro's first audit run died on all unpinned spawns and only succeeded after pinning `model: 'sonnet'` at all 4 call sites). *(AC added at the sprint-2026-06-28 retro Phase 5 gate, 2026-07-06 — relocated here from `retro-audit-workflow-process-findings-return` because this story owns the `agent()` call sites.)*

## Tasks / Subtasks

- [ ] **Task 1 — Author the audit Workflow script** (AC: 2, 3, 4, 7, 8, 9, 13) — *change type: script-code*
  - [ ] Pin `model: 'sonnet'` (explicit `model:` opt) on every `agent()` call in the script (AC13 — [1m]-session entitlement guard; see memory `reference_1m_context_subagent_entitlement`).
