...
## What
momentum/agents.json defaults map architect, pm, sm AND ux, analyst, researcher to agent body files that do NOT exist (verified: only dev.md, qa-reviewer.md, e2e-validator.md plus dev-build/dev-frontend/dev-skills exist; this is BROADER than the lenses' architect/pm/sm claim — ux/analyst/researcher are also dead). cmd_agent_resolve reads agents.json and returns the mapped path with NO Path.exists() check, so `agent resolve --role architect` returns success:true pointing at a missing file. Spec §10/§12 call this P2 cleanup: create the bodies or delete the dead entries, and ideally add existence validation so a broken path fails fast instead of silently mis-spawning.
