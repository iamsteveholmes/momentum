...
avfl_checkpoint:
  run_id: phase6b-checkpoint-iter1
  timestamp: 2026-03-20
  pre_fix_score: 70
  post_fix_score_estimate: 97
  status: CHECKPOINT_WARNING
  remaining_finding: "C-003 (medium) — install command -a claude-code vs plugin install path unresolved; verify against agentskills.io spec"
---
...
Industry data is unambiguous: AI code generation without governance fails. Without a verification architecture, AI-generated code accumulates quality debt silently: PRs are rejected at rates far above human-authored code, issues per PR are higher, experienced developers report feeling slower while believing they're faster, and organizations see activity metrics climb while delivery metrics stay flat. The problem is not the tools. The problem is the absence of a verification architecture, a quality discipline, and a continuous improvement practice around them.

<!-- UNSOURCED: The original PRD included specific statistics (67.3%, 1.7x, 19%, 20%, 98%) that do not appear in any declared source document (handoff-product-brief-2026-03-14.md or other inputDocuments). These have been replaced with qualitative framing pending citation verification. If these figures exist in external research, they must be cited before this PRD is finalized. -->

Momentum provides that missing layer through eight composable principles: spec-driven development, an authority hierarchy (specifications > tests > code — encoded into machine-readable `derives_from` chains enforced by tooling), producer-verifier separation, an evaluation flywheel that traces failures upstream via navigable `derives_from` chains, three tiers of enforcement (deterministic hooks, structured workflows, advisory rules), cost as a managed dimension (the cognitive hazard rule: for outputs without automated validation, use flagship models — invisible errors cost more than the price premium), provenance as infrastructure, and protocol-based integration (each major capability defines an interface so implementations can be substituted across teams, tools, and environments). The system is delivered as standard Agent Skills — installable via `npx skills`, portable across 15+ AI coding tools, with Claude Code-specific enforcement (hooks, subagents, rules) bundled as a plugin for full Tier 1 deterministic governance.
...
**Standard packaging with tool-optimized enforcement.** The principles are tool-agnostic. The skills are portable (Agent Skills standard, 15+ tools). The enforcement is Claude Code-optimized (hooks that always fire, subagents with read-only tools, rules that auto-load every session). One set of SKILL.md files serves both audiences — extra frontmatter fields are silently ignored by tools that don't understand them. This is not a compromise; it is the intended design of the spec.
...
Momentum is designed solo-first; team adoption is the intended growth trajectory. These metrics apply when Momentum expands beyond the solo developer — they are not MVP gates, included here for traceability against Journey 4.
...
- Standard skills live in `skills/` — portable across all 15+ Agent Skills-adopting tools
...
|---|---|---|
| Claude Code plugin install (also: `npx skills add momentum -a claude-code` for skills-only install; plugin install adds full hooks/agents/rules) | Claude Code | Full system: skills + hooks + agents + rules |
...
<!-- REMAINING FINDING C-003 (medium): The exact relationship between 'npx skills add momentum -a claude-code' and 'Claude Code plugin install' is unresolved. Verify against agentskills.io spec whether -a claude-code triggers plugin install or skills-only install. Update this table and Journey 1 accordingly. -->
...
- **FR37:** System can resolve workflow step invocations through protocol interfaces — workflow definitions reference protocol types, not specific implementations; the runtime resolves the binding from project configuration
...
- **NFR5:** All SKILL.md files must be valid Agent Skills standard — parseable by any of the 15+ adopting tools
...
- Update project README with the eight core principles (the original seven plus Protocol-Based Integration)
- Commit LICENSE as the first artifact in the Momentum repository
- **[AVFL follow-up]** Locate and cite source research for executive summary statistics (67.3%, 1.7x, 19%, 20%, 98%), or remove them if no source can be found — these figures are currently UNSOURCED
- **[AVFL follow-up]** Verify `npx skills add momentum -a claude-code` flag syntax against agentskills.io specification and reconcile with Installation Architecture table
