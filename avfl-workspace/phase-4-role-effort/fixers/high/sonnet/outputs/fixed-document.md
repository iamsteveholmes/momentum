...
Authentication is enforced at the ingestion boundary via API key validation on every inbound HTTP request. All data in transit is encrypted using TLS. Data at rest in the message queue and data warehouse is encrypted using AES-256. Access to the metrics endpoint and internal queue is restricted to authorized services within the private network perimeter.
