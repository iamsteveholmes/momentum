...
| **merge-resolver** — resolves worktree→sprint and sprint→main conflicts | nothing | **MISSING** | **P1 (decision 9).** | No new base body. Conductor resolves trivial conflicts inline; for semantic, fires the **directed fixer (momentum:dev fix-mode)** as the conflict-resolution worker (it has Edit/Bash [Superseded 2026-06-12 — see §6 note]/commit). Merge failure → retry. |
