...
- **FR62:** Sprint execution reads the activated sprint record, creates a task list for progress tracking, spawns `momentum:dev` agents for unblocked stories (each in its own worktree), tracks completion via tasks, handles dependency-driven sequencing, and surfaces a sprint summary. Every merge requires explicit developer confirmation. Post-merge quality runs in two sequential phases: (a) AVFL scan mode produces a scored findings list (all 4 lenses, dual reviewers, zero fix iterations); (b) a concurrent Agent Team resolves the findings — Dev fixes AVFL findings, QA validates story ACs, E2E Validator tests running behavior against Gherkin specs using external tools, Architect Guard checks pattern drift. **Priority: High** *(Phase 3)*
