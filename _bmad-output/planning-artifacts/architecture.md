...
editHistory:
  - date: '2026-08-02'
    changes: 'DEC-040 spec impact (Delivery Loop Closure): Amended the features.json note in .momentum/ State Layout — features.json returns as a standalone per-project ACCEPTANCE-TESTING registry at .momentum/features.json (DEC-040 D1, amending DEC-034): failing-by-default statuses (absent|failing|passing), human-only verdicts with build SHAs recorded via momentum-tools feature verify (sole writer), integration limited to exactly three seams (sprint-planning goal_conditions declaration, sprint-closure verdict, Impetus N-passing report); epics.json remains the work-tracking layer unchanged, and no epic-facing skill reads features. Extended the Sprint goal field paragraph (Sprint Tracking Schema) with goal_conditions (DEC-040 D3) — 0..n feature slugs whose acceptance conditions the sprint intends to flip; maintenance sprints declare a verified-fix list instead; added goal_conditions to the sprints/index.json schema example. Rewrote the Decision 59 dual end-gate verdict paragraph as the DEC-040 D2/D3 two-tier gate: the MERGE gate ("code we are comfortable keeping") completes only by executing merge + push + story status transitions and FAILS loudly otherwise (terminal state is a state transition, never a report); SPRINT CLOSURE is a separate, possibly later decision requiring a recorded human PASS/FAIL per goal condition against a paired build, with an honest FAILED terminal state available; condition-serving discovered work defaults INTO the sprint per the DEC-039 D2 routing matrix (punting goal-critical work requires an explicit recorded decision); gate-pending surfaces follow the DEC-040 D5 resume-card contract — session-start re-surfacing with rising prominence, never expiry-FAIL, wall-of-text surfaces are a defect. Added momentum-tools feature verify row to the Read/Write Authority table (sole writer of .momentum/features.json). Conductor/sprint-planning/Impetus implementation stories pending — see DEC-040 Phased Implementation Plan.'
  - date: '2026-07-13'
    changes: 'Sprint-2026-07-13 spec impact (DEC-039 fork resolution — pre-sprint architecture pass, sprint-planning Step 4.5): Added the `goal` field (DEC-039 D1) to the Sprint Tracking Schema — a one-to-few-sentence sprint goal string captured at planning, stored on `active`/`planning`/`completed` sprint-record entries, approved at the plan gate (Decision 29 Step 7); added to the `.momentum/sprints/index.json` schema example. Added the DEC-039 D2 goal-criticality routing matrix as a table under Sprint Orchestration Architecture — four tiers (not goal-critical → triage punt; goal-critical + designed → citation-gated auto-pull; goal-critical + undesigned/upstream-touching → pause-ask escalation; goal-critical + oversized → conductor-recommended new sprint) — marked amendment-pending (conductor-side machinery not yet implemented). Added the DEC-039 D3 dual end-gate verdict paragraph to Decision 59 — stories-delivered and goal-delivered as two distinct verdicts; a sprint can pass the first and fail the second; tier-(d) punts of goal-critical work fail the goal verdict and obligate retro root-cause — also marked amendment-pending. Added the cross-story coherence gate to the Sprint Planning Workflow — a deterministic seam-coherence check (after Step 4 contract authoring, before Step 8 activation) verifying every intra-sprint `depends_on` edge''s producer story spec promises what the consumer story''s spec expects; unresolved mismatches block activation absent an explicit recorded developer override (`sprint-planning-cross-story-coherence-gate` story). Extended sprint-planning Step 8 (Sprint activation) with the mandatory terminal build-handoff artifact — a use-then-discard markdown handoff at `.momentum/handoffs/` per the handoff-conventions rule, carrying the sprint goal, stories by wave, contract locations, and planning cautions (`sprint-planning-handoff-artifact` story). Updated qa-reviewer''s description across the Three-Layer Agent Model, Skills Deployment Classification, Decision 34 Application table, and Decision 59 stage-2 — qa-reviewer becomes harness-driven and method-polymorphic, routing on the frozen contract''s `verification_method`/`harness_profile` and **executing** the routed method (build+launch+drive for smoke, or BLOCKED when the environment cannot support it), matching the execution model previously unique to e2e-validator (`conduct-qa-execute-verification-method` story).'
  - date: '2026-07-06'
    changes: 'Sprint-2026-06-28 spec impact (decision-grade companion surfaces + deterministic file ownership): Extended Decision 29 Step 7 (Developer review) — the final sprint-planning step now mandatorily EMITS a visual-HTML companion plan gate at .momentum/handoffs/{sprint_slug}-plan-gate.html (sibling of the conduct endgate-report.html): synthesis-now render carrying a plain-language purpose hero, an inline-SVG dependency/wave diagram marking any single-point-of-failure story, a scannable per-story table, ≤7 genuine-fork What/Why/Evidence/Recommendation/Options decision cards, and an anti-rubber-stamp sign-off; links to (never inlines) canonical .momentum/stories/{slug}.md files. Added a Conduct ↔ retro fused post-sprint surface block to the Retro → Planning Handoff section — retro''s ≤7 Keep/Stop/Change process digest folds into conduct''s endgate-report.html as ONE fused, results-first surface (single post-sprint surface invariant; retro owns the fused re-render off the Conductor''s persisted end-gate data model). Updated Decision 59: added a Fused post-sprint results surface paragraph; broadened the Presentation standard (DEC-036 D5) note''s documented scope from "conduct end-gate + conversational skills" to also carry the decision-grade-presentation §5 companion-surface obligation and §5.1 presentation-form leg, both governing the pre-sprint plan gate (Decision 29) and the post-sprint results gate; rewrote the Carried forward note — plan-gate legibility is promoted from a deferred/backlog item (formerly DEC-035 D7 / DEC-036 D6) to a REQUIRED emission of sprint-planning''s final step (Decision 29). Added the normative ## File Ownership field to the Decision 56 manifesto content model — a machine-readable list of glob strings that is the deterministic source of the composed agent''s agents.json patterns (read verbatim by build-guidelines), replacing best-effort LLM inference from ## Project Stack prose; a manifest lacking the field is incomplete and surfaces an explicit signal (closes DEC-038 Gate G1 deterministic reachability).'
  - date: '2026-06-11'
...
7. **Orchestrating Agent — Impetus** — Session orientation via 9-state greeting (Decision 37) with adaptive menus, visual progress (✓ Built / → Now / ◦ Next) for workflow phases, proactive gap detection, productive waiting during subagent execution, hub-and-spoke voice unification, narrative voice contract (Decision 38). Impetus is the primary entry-point skill (`/momentum:impetus`) and the recommended path for all Momentum operations. Users can invoke other namespaced skills directly (e.g., `/momentum:sprint-planning`), but Impetus provides session orientation and context that direct invocation skips. For sprint-scoped operations, Impetus dispatches to dedicated skills: `/momentum:sprint-planning` for story selection and team composition, and `/momentum:conduct` for sprint-build execution. **Per DEC-037, `/momentum:conduct` (the Conductor orchestrator — Decision 59) is the primary build path; `/momentum:sprint-dev` is the legacy builder, still invocable directly but not the default dispatch target for new "run the sprint build" requests.** When invoked directly as the top-level session, `/momentum:conduct` legitimately owns commits, merges, and the approve-time `git push` (orchestrator-purity forbids only *spawned* subagents from mutating git — see Decision 59). Sub-command dispatch: developer selects from the greeting menu (e.g., "Continue the sprint", "Run retro", "Plan a sprint"), and Impetus invokes the corresponding skill. Sprint lifecycle transitions follow Decision 36 state machine. Impetus is the force that maintains practice velocity — the system keeps compounding because Impetus carries knowledge and context forward across sessions and sprints without requiring repeated external input.

<!-- REVISED 2026-07-06: Conductor primary dispatch path, sprint-dev legacy — updated per project rule .claude/rules/impetus.md and DEC-037 D1. -->
...
Momentum's operational runtime state lives under a single hidden top-level directory: `.momentum/` at the project root. This directory holds everything the practice itself produces and consumes during normal operation — sprint records, story index, signals, the intake queue, and per-sprint working subtrees. Planning artifacts that describe what to build (PRD, architecture, epics, decisions, assessments) remain under `_bmad-output/planning-artifacts/` — that carve-out is intentional. Note: `features.json` was the prior planning artifact for feature taxonomy; for **work tracking** it is superseded by `epics.json` per DEC-034 (2026-05-25). Per DEC-040 D1 (2026-08-02), `features.json` returns at `.momentum/features.json` as a standalone **acceptance-testing registry** — failing-by-default feature statuses (`absent | failing | passing`) with per-feature acceptance conditions and human-only verdicts (build SHAs recorded) — deliberately distinct from the epic layer. It integrates at exactly three seams (sprint-planning `goal_conditions`, sprint-closure verdict via `momentum-tools feature verify`, Impetus N-passing reporting); no epic-facing skill reads it.
...
| QA reviewer | Agent definition file (`agents/qa-reviewer.md`) | Pure spawned worker — harness-driven and method-polymorphic (sprint-2026-07-13, `conduct-qa-execute-verification-method` story, matching the execution model previously unique to e2e-validator). Routes on the frozen contract's `verification_method`/`harness_profile` and **executes** the routed method (e.g. build+launch+drive for `smoke`), reporting BLOCKED when the environment cannot support it. Runs during Team Review (Decision 34) / conduct stage 2 (Decision 59). Never user-invoked (Decision 35). |
...
│   │   └── SKILL.md
│   ├── constitution-builder/                ← /momentum:constitution-builder (Decision 56 / DEC-026 D3 — Tier 1: hot constitution; write-mode parameterized; no ## Quick Routing per DEC-038)
│   │   └── SKILL.md
...
| momentum-tools feature verify | `.momentum/features.json` | **Sole writer of `.momentum/features.json`** (DEC-040 D1) — records human PASS/FAIL verdicts with paired-build SHAs; no agent or skill may write feature status by any other path. Read-only consumers: sprint-planning (`goal_conditions` declaration), Impetus (N-passing headline report), daily tasting-brief generator (DEC-040 D4) |
...
**`.momentum/sprints/index.json`** — which sprint is active, which is planning, list of completed sprints. Active and planning entries are objects (not slug strings) that carry sprint lifecycle state (Decision 36). The `status` field tracks position in the sprint lifecycle state machine. Active and planning entries also carry an `approvals: []` array (Per-story approval contract — added 2026-04-28) and a `goal` field (Sprint goal field — added 2026-07-13, DEC-039 D1). Completed entries track retro execution for lifecycle gate enforcement and retain `goal` for historical reference.
...
**Sprint goal field (added 2026-07-13, DEC-039 D1):** `active`, `planning`, and `completed` sprint-record entries carry a `goal` field — a one-to-few-sentence string naming what the sprint delivers. It is captured during sprint-planning (Steps 1–2, Backlog presentation / Story selection) and approved by the developer at the plan gate (Sprint Planning Workflow Step 7 — Developer review). The `goal` field is the anchor read by two downstream mechanisms: the DEC-039 D2 goal-criticality routing matrix (Sprint Orchestration Architecture), which classifies conduct-discovered work as goal-critical or not against this field, and the two-tier gate (Decision 59, DEC-040 D2/D3), which renders the sprint-closure verdict against it. Write authority is unchanged — `momentum-tools.py sprint` is the sole writer, same as the rest of the sprint record.

**Sprint goal_conditions field (added 2026-08-02, DEC-040 D3):** sprint-record entries additionally carry `goal_conditions` — an array of 0..n feature slugs from `.momentum/features.json` whose acceptance conditions this sprint intends to flip from failing to passing. The plan gate's first fork presents them in plain language ("after this sprint a GM will be able to ___ — approve/change/reject"). A sprint with no feature-shaped goal (bug-fix or configuration sprints) declares an explicit verified-fix list in its `goal` instead; closure then requires each named fix confirmed in the running app. Sprint closure (distinct from the merge gate per DEC-040 D2) requires a recorded human PASS/FAIL per named condition against a correctly-paired build — recorded via `momentum-tools feature verify` — or the sprint closes with the honest terminal state FAILED. Discovered work that serves a named condition defaults into the sprint (loud absorption per DEC-039 D2); punting goal-critical work to a future sprint requires an explicit recorded decision.
...
// .momentum/sprints/index.json
{
  "active": {
    ...
    "locked": true,
    "goal": "Ship the three quality-enforcement hooks and close the lint/format gap in CI.",  // DEC-039 D1 — captured at planning, approved at the plan gate
    "goal_conditions": ["session-capture", "campaign-identity"],  // DEC-040 D3 — feature slugs whose acceptance conditions this sprint flips; [] for maintenance sprints (verified-fix list lives in goal)
  ...
  "planning": {
    ...
    "locked": false,
    "goal": "Redesign the Impetus greeting to carry personality and session stats.",  // DEC-039 D1
  ...
  "completed": [
    {
      ...
      "goal": "Stand up the initial project scaffolding and story lifecycle.",  // DEC-039 D1 — retained for retro root-cause / goal-verdict history
...
<!-- REVISED 2026-07-06: Primary execution engine is now /momentum:conduct (Conductor orchestrator — Decision 59 / DEC-035); /momentum:sprint-dev is legacy builder. Both coexist during transition; sprint-dev wave loop retired at later adoption step. -->
...
**Goal-criticality routing matrix (DEC-039 D2):** conduct-discovered work — anything found necessary mid-build that wasn't a planned story — is routed against the sprint's `goal` field (Sprint Tracking Schema, DEC-039 D1) by the following four-tier matrix:

| Tier | Condition | Routing | Notes |
|---|---|---|---|
| (a) | Not goal-critical | Punt to backlog via `momentum:triage` | Standard out-of-scope handling — no sprint disruption, no escalation |
| (b) | Goal-critical **and** already designed | Auto-pull into the active sprint, autonomously | Mandatory covering-artifact citation (the design/architecture/PRD passage that already accounts for the work); recorded as a `discovered_from` edge; reported at the end-gate |
| (c) | Goal-critical **and** undesigned, or requires upstream artifact changes | Pause-ask escalation | Uses the DEC-036 Pause-Ask Surface Contract (mid-flight escalation) |
| (d) | Goal-critical **and** too large for the active sprint | Conductor recommends a new sprint | Developer decides; a tier-(d) punt of goal-critical work **fails the DEC-039 D3 goal verdict** and **obligates retro root-cause** (see Decision 59) |

**Implementation status:** conductor-side machinery for this routing is **not yet implemented** — the matrix is applied **manually** by the Conductor until the machinery lands via stories (sprint-planning, conductor, sprint-manager, retro).
...
7. **Developer review** — present the full plan for approval; developer can request adjustments. This final step **mandatorily emits a visual-HTML companion plan gate** at `.momentum/handoffs/{sprint_slug}-plan-gate.html` — a new human-facing decision surface, sibling of the conduct end-gate report (`endgate-report.html`, Decision 59). The gate is generated **synthesis-now** (rendered at this step, not deferred) and, per the decision-grade-presentation standard's §5 companion-surface obligation + §5.1 presentation-form leg, carries: a **plain-language purpose hero** (what the sprint accomplishes, before any table); an **inline-SVG dependency/wave diagram** marking any single-point-of-failure story (a node whose failure blocks the rest); a **scannable per-story table**; **≤ 7 genuine-fork decision cards**, each a What/Why/Evidence/Recommendation/Options card (per the Pause-Ask template — everything defaultable is defaulted silently to standards); and an **anti-rubber-stamp sign-off** (a blanket "approve all" is insufficient — each genuine fork forces a one-line written verdict + reason). The gate **links to, never inlines,** the canonical story files (`.momentum/stories/{slug}.md`) — those stay the implementing agent's source of truth; the gate is the human's review artifact, the large story set is its depth-on-demand backing.
8. **Sprint activation** — call `momentum-tools sprint activate`; log the decision. Activation also **mandatorily emits a terminal build-handoff artifact** — a use-then-discard markdown handoff written to `.momentum/handoffs/` per the handoff-conventions rule (filename `<topic>-<YYYY-MM-DD>.md`), carrying the sprint goal (DEC-039 D1), the story list organized by wave, contract file locations per story, and any planning-time cautions the build phase should carry forward. This is a bridging document consumed by the next `momentum:conduct` / `momentum:sprint-dev` build session — not a durable record; it is discarded after consumption like other handoffs. Source: `sprint-planning-handoff-artifact` story (sprint-2026-07-13).

**Cross-story coherence gate (source: `sprint-planning-cross-story-coherence-gate` story, sprint-2026-07-13):** a deterministic seam-coherence check runs after Step 4 (contract authoring) and before Step 8 (activation). For every intra-sprint `depends_on` edge in the planned stories, the gate verifies that the **producer** story's spec/contract actually promises the deliverable the **consumer** story's spec expects to consume — catching seam mismatches (e.g., a consumer story assuming an API shape or artifact the producer story never commits to) before they surface mid-build as conduct findings. Unresolved mismatches **block activation** unless the developer records an explicit override at the plan gate (Step 7).
...
### Three-Layer Agent Model (Decision 56 / DEC-038)

<!-- REVISED 2026-07-06: Updated from two-layer to three-layer per Decision 56 / DEC-038. The three layers are: (1) base body, (2) constitution, (3) manifesto (diagnostic table). Routing table resolution (Decision 55 / DEC-023) maps stories to specialists via glob-match against momentum/agents.json. -->

Momentum provides generic agent roles with orchestration patterns:
- **Dev** — implements stories in worktrees
- **QA** — harness-driven, method-polymorphic verifier: routes on the frozen contract's `verification_method`/`harness_profile` and **executes** the routed method (e.g. build+launch+drive for `smoke`), reporting BLOCKED when the environment cannot support it (sprint-2026-07-13, `conduct-qa-execute-verification-method` story — matches the execution model previously unique to E2E Validator)
...
The three-layer composition model assembles project-specific specialist agents from: (1) plugin-shipped **base body** per role, (2) project-scoped **constitution** (identity, values, constraints), and (3) per-role×domain **manifesto** (the diagnostic table: observable developer symptom → `wiki-query` KB lookup). The routing table (`momentum/agents.json`, Decision 55) resolves which agent handles a story via glob-match on `change_type` and `touches`. Sprint planning uses this resolution to determine which roles apply to each story, then attaches the composed agent file for that role. The team composition is stored in the sprint record.
...
**Conduct ↔ retro fused post-sprint surface (sprint-2026-06-28):** the post-sprint results gate is **one fused, results-first HTML surface**, not two competing artifacts. Retro's ≤ 7 Keep/Stop/Change process digest **folds into** conduct's `endgate-report.html` (Decision 59) rather than rendering as a separate document — enforcing the **single post-sprint surface invariant**: the developer is handed exactly one surface after a build. **Retro owns the fused re-render** — it reads conduct's persisted end-gate data model and re-renders the combined surface with its process digest merged in (results — what shipped, divergences, fixer actions — leading; the process Keep/Stop/Change digest following). This is the companion-surface obligation (decision-grade-presentation §5) applied to the post-sprint gate: the fused surface is the human's review artifact; the build ledger and full retro findings are its depth-on-demand backing.
...
| QA reviewer | Agent definition file (`agents/qa-reviewer.md`) | Pure worker: method-polymorphic and harness-driven (sprint-2026-07-13, `conduct-qa-execute-verification-method` story). Routes on the frozen contract's `verification_method`/`harness_profile` and **executes** the routed method (build+launch+drive for `smoke`), reporting BLOCKED when the environment cannot support it — matching the execution model previously unique to E2E Validator. Needs Bash for method execution. Never user-invoked. |
...
**Routing ownership — per-agent, not shared constitution (DEC-038):** Decisions 55 and 56 did not state *where* per-agent routing is owned. It is now explicit: **agent-specific routing is owned at the manifesto / agent-builder layer (per-agent, per role×domain)** — it is the manifesto's diagnostic table — **not** the shared, project-wide constitution. The routing table (`momentum/agents.json`, Decision 55) still resolves *which agent* handles a story; *within* an agent, the symptom→`wiki-query` routing is the manifesto. This resolves an open design conflict: constitution-builder currently writes a project-**shared** `## Quick Routing` block, but a single shared Compose/Kotest routing table is **meaningless for a `pm` or `architect`** whose domain has different symptoms and KB pages. **Resolution (ratified, DEC-038 Phase 1 — stories `constitution-builder-write-mode-parameterization` AC7 and `constitutionmd-generation-acceptance-criteria`):** symptom→`wiki-query` routing belongs in the **per-agent manifesto (diagnostic table)**, not the shared constitution. `constitution-builder` emits **no `## Quick Routing`** section under any write mode — routing generation is removed from constitution-builder and owned by the manifesto / `agent-builder` layer. The shared constitution may still carry project-universal, agent-agnostic content (embedded facts, KB-sourced context, the wiki-query interface block); it carries no per-agent routing.
...
- **`## File Ownership` — normative content-model field (sprint-2026-06-28):** the manifesto content model now carries a required **`## File Ownership`** section — a **machine-readable list of glob strings** naming the files/paths the composed agent owns. This list is the **deterministic source** of the composed agent's `agents.json` `patterns` entry: `build-guidelines` reads it **verbatim** into the routing table, **replacing** the prior best-effort LLM inference of ownership globs from `## Project Stack` prose. A manifest **lacking `## File Ownership` is incomplete** — the composition surfaces an **explicit signal** rather than silently guessing patterns. This closes **DEC-038 Gate G1 (deterministic reachability)**: the routing table's `patterns` are now derived from a declared, checkable field instead of inferred, so every agent's reachable file scope is deterministic and auditable.
...
**Two-tier gate (DEC-040 D2/D3, superseding the DEC-039 D3 single dual-verdict gate):** the end of a conduct build resolves through **two independent gates asking two different questions, decided separately and possibly days apart**. **Tier 1 — the merge gate** ("is this code something we are comfortable keeping on trunk?"): completes **only by executing** the state transition — merge to main + push + story status updates — and **FAILS loudly** if it cannot; its terminal state is never a rendered report (the sprint-2026-07-13 lesson: an approval that leaves commits stranded is an opinion, not an approval). **Tier 2 — sprint closure** ("have the sprint goals been met — do the named conditions verifiably work in the app?"): requires a recorded **human** PASS/FAIL per `goal_conditions` entry against a correctly-paired build (recorded via `momentum-tools feature verify`), or the sprint closes with the honest terminal state **FAILED**. A sprint can pass tier 1 and remain open at tier 2 indefinitely while condition-serving discovered work is absorbed into it (DEC-039 D2 loud-absorption mechanism; punting goal-critical work requires an explicit recorded decision, never a default). Gate-pending surfaces follow the **resume-card contract (DEC-040 D5)**: when a build finishes, the Conductor emits a resume card — where we were, what merged, the one question pending — and Impetus leads the next session with that card as the agenda; escalation is session-start re-surfacing with rising prominence, **never expiry-FAIL** (the practice serves a nights-and-weekends developer); a wall-of-text gate surface is a defect under the Decision-Grade Presentation Standard. Any punt of goal-critical discovered work still **obligates a retro root-cause** (carried forward from DEC-039 D3). **Implementation status:** conductor/sprint-manager/Impetus machinery **not yet implemented** — applied manually until DEC-040 Phase 2 stories land.
...
**Stage-2 qa-reviewer execution model (sprint-2026-07-13, `conduct-qa-execute-verification-method` story):** the QA reviewer spawned at stage 2 is now **harness-driven and method-polymorphic**, matching the execution model previously unique to e2e-validator. It routes on the story's frozen contract `verification_method`/`harness_profile` (Sprint Tracking Schema `story_assignments` frozen-contract block) and **executes** the routed method rather than only reading the diff against ACs — e.g. build+launch+drive for a `smoke` contract. When the environment cannot support the routed method (missing runtime, unreachable dependency, etc.), qa-reviewer reports **BLOCKED** rather than guessing at a verdict. This execution behavior feeds the stage-2 finding normalization above unchanged — BLOCKED still maps to `critical` severity.
...
**Presentation standard (DEC-036 D5; scope broadened sprint-2026-06-28):** the end-gate template and conduct's human-facing output follow the practice-wide **decision-grade presentation** rule (in-project: `.claude/rules/decision-grade-presentation.md`; plugin-shipped: `skills/momentum/references/rules/decision-grade-presentation.md`, Rules Architecture subsystem) — measurable caps govern verbosity, `effort` drives depth, and a self-sufficiency floor guarantees each decision carries its `what / why / evidence` inline. As of sprint-2026-06-28 the standard's **documented scope broadens** from "conduct end-gate + conversational skills" to also carry the **§5 companion-surface obligation** (a process that emits a large review document must also emit a paired decision surface) and the **§5.1 presentation-form leg** (decision gates render as visual, purpose-first, diagrammed HTML). Both legs now govern **both** the pre-sprint plan gate (Decision 29) and the post-sprint results gate (the fused surface below) — the two named companion-surface instances. The rule establishes the four-part standard framework: (1) measurable caps on output verbosity (≤N bullets, word budgets), (2) self-sufficiency floor requiring what/why/evidence inline, (3) companion-surface obligation pairing review documents with decision surfaces, (4) presentation-form leg requiring visual HTML for decision gates.

<!-- REVISED 2026-07-06: Added cross-reference to decision-grade-presentation.md rule locations (both in-project and plugin-shipped); clarified the four-part standard framework. -->

**Fused post-sprint results surface (sprint-2026-06-28):** the post-sprint gate is **one fused, results-first surface**, honoring a **single post-sprint surface invariant** — the developer is handed exactly one artifact after a build, never a conduct report competing with a separate retro document. Retro's ≤ 7 Keep/Stop/Change process digest **folds into** this `endgate-report.html`; **retro owns the fused re-render** (it reads the Conductor's persisted end-gate data model and re-renders the combined surface, results leading, process digest following). See the Retro → Planning Handoff section for the data-flow detail.

**Carried forward (not decided here):** **plan-gate legibility is no longer deferred** — as of sprint-2026-06-28 the pre-sprint plan gate is promoted from a backlog/deferred item (formerly DEC-035 D7 / DEC-036 D6) to a **required emission** of sprint-planning's final step (Decision 29): the visual-HTML companion plan gate at `.momentum/handoffs/{sprint_slug}-plan-gate.html` is mandatory, not optional. Conductor context-budget handling remains a build implementation detail (subagents do heavy work in their own contexts and return structured results; the report renders from a persisted data model). DEC-030 Gate 4 is validated in the first real conduct run (DEC-035 D8).
