...
|---|---|---|
| **Producer** | `momentum:dev` fix-mode (fixer subagent) | Emits per-finding disposition objects; populates `finding_id`, `disposition`, `files_changed`, `dismissal_rationale`, and the nested `escalation` object |
