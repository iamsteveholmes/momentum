...
I want the practice-ledger to be a true append-only event log with a fixed-read reader CLI (pure-Python fold today; arbitrary-SQL querying via DuckDB DEFERRED to follow-up story `practice-ledger-duckdb-sql-query-command`), first-class `event_id`/`entity_id` distinction, and a 15-day TTL closure path,
...
5. **Fixed-read reader CLI (DEC-033 D7):** `momentum-tools practice-ledger` subcommands — `summary`, `open`, `history --entity <id>`, `since <ts>`, `by-source <source>`. Readers compute state by a pure-Python fold (`_load_ledger_events`); nothing is stored. D7's "DuckDB-backed" wording refers to an arbitrary-SQL-query interface that is DEFERRED to follow-up story `practice-ledger-duckdb-sql-query-command`; the five fixed subcommands above need no DuckDB and are shipped.
...
15. **PARTIALLY SATISFIED.** The five fixed reader subcommands (`summary`, `open`, `history`, `since`, `by-source`) are implemented via a pure-Python fold (`_load_ledger_events`): glob over `.momentum/practice-ledger*.jsonl`, per-line JSON parse, then fold events by `entity_id` to derive current state. State is never stored — derivation is the source of truth, and these subcommands are fully working and tested. The DuckDB-backed arbitrary-SQL-query interface originally implied by this AC (the real value DuckDB would add) is DEFERRED to follow-up story `practice-ledger-duckdb-sql-query-command` (filed high priority). No `duckdb` import exists in the current implementation.
...
- [x] **Task 4 — Fixed-read reader CLI (script-cli).** Implement subcommand parser group `practice-ledger` with `summary`, `open`, `history --entity`, `since`, `by-source`. Each subcommand reads `.momentum/practice-ledger*.jsonl`, folds events by `entity_id` to derive current state, and emits JSON (default) or text (with `--format text`). TDD: red tests for each subcommand against a seeded fixture ledger; green by implementing the fold. Pre-2026-05 archive entries must be tolerated — entries that fail new-schema parsing count as `archive_entries` and do not crash the reader. **As built:** implemented as a pure-Python fold (`_load_ledger_events`: glob + per-line JSON parse + `entity_id` fold) — no `duckdb` import. The "DuckDB" in the original task title referred to an arbitrary-SQL-query interface, which is DEFERRED to follow-up story `practice-ledger-duckdb-sql-query-command`.
- [x] **Task 5 — Close-stale subcommand (script-cli).** Implement `practice-ledger close-stale --age-days <N>` (default 15). Identify entities whose current event is non-terminal AND whose `created` event `ts` is older than `N` days. For each, append one `closed_stale` event with `source: momentum-tools-close-stale`. TDD: red tests for idempotency (second invocation appends zero events) and for the boundary condition (entity exactly at TTL — include `>` not `>=` so exactly-at-TTL is not closed). **As built:** uses the same pure-Python fold as Task 4 (no DuckDB).
...
- **Reader implementation note** — the fixed-read subcommands use a pure-Python glob + per-line JSON parse + `entity_id` fold (`_load_ledger_events`); no DuckDB. DuckDB is available in the project's dependency set per DEC-027 (retro transcript audit), but it is reserved for the DEFERRED arbitrary-SQL-query interface in follow-up story `practice-ledger-duckdb-sql-query-command` — it is not used by these five fixed reads.
