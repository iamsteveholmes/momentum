...
<workflow>
  ...
  <step n="3" goal="Classify change types from story tasks">
    ...
    <action>Load ./references/change-types.md for detection heuristics</action>
    <action>For each task, apply the detection signals to classify it as one of:
      - skill-instruction: SKILL.md, workflow.md, references/, skill instructions, agent definitions
      - script-code: .sh, .py, .ts, scripts/, bin/, executable files
      - rule-hook: .claude/rules/, settings.json hooks, frontmatter config entries
      - config-structure: JSON configs, directory structure, version files, momentum-versions.json, installed.json
    </action>
    <action>Produce a classification list: each task tagged with its change type. A story may contain multiple types.</action>
  ...
  <step n="4" goal="Inject Momentum Implementation Guide into story Dev Notes">
    ...
    <action>Load ./references/change-types.md to get the injection template for each detected type</action>
    <action>Compose the Momentum Implementation Guide section using the templates for all detected types in this story</action>
    <action>Inject the section at the END of the Dev Notes / Developer Context section, immediately before the Dev Agent Record section</action>
  ...
  <step n="5" goal="Run AVFL checkpoint on the story file">
    <action>Invoke the `avfl` skill with these parameters:
      - domain_expert: "BMAD story context engine"
...
Next: run momentum-dev to implement.</output>
  </step>
