...
**Presentation standard (DEC-036 D5; scope broadened sprint-2026-06-28):** the end-gate template and conduct's human-facing output follow the practice-wide **decision-grade presentation** rule (`references/rules/decision-grade-presentation.md`, Rules Architecture subsystem) — measurable caps govern verbosity, `effort` drives depth, and a self-sufficiency floor guarantees each decision carries its `what / why / evidence` inline. As of sprint-2026-06-28 the standard's **documented scope broadens** from "conduct end-gate + conversational skills" to also carry the **§5 companion-surface obligation** (a process that emits a large review document must also emit a paired decision surface) and the **§5.1 presentation-form leg** (decision gates render as visual, purpose-first, diagrammed HTML). Both legs now govern **both** the pre-sprint plan gate (Decision 29) and the post-sprint results gate (the fused surface below) — the two named companion-surface instances.
