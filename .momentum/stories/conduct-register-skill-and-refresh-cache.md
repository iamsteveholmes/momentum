...
The skill exists on disk (shipped in the installed plugin source) but is not in the registry the session can reach, so neither /momentum:conductor nor the Skill tool can invoke it without a marketplace refresh. Even a correct command file won't dispatch until the skill registers.
