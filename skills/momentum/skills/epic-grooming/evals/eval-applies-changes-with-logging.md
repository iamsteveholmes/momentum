...
behavior: Approved changes update epics.md and reassign stories via momentum-tools, with documented completion notes
---

# Eval: Applies Approved Changes With Full Documentation
...
3. Document every approved change in the Dev Agent Record completion notes, capturing old slug, new slug, and rationale for each change.
...
- Every decision is documented in completion notes with structured detail including old_slug, new_slug, and rationale
...
- Skip documenting any approved change
- Apply rejected changes
- Call `momentum-tools log` (agent journal logging has been removed project-wide)
