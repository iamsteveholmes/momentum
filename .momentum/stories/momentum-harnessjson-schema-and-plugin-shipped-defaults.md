...
I want a `momentum/harness.json` file (plugin-shipped defaults, project-overridable) that declares how acceptance validation is driven,
...
1. A file `momentum/harness.json` exists in the plugin root, structurally parallel to `momentum/agents.json`: a top-level JSON object with exactly two keys — `"defaults"` (object, plugin-shipped) and `"project"` (array, initially empty).
...
6. The `agent-builder` workflow is updated so that when it writes a new agent's routing entry to `momentum/agents.json`, it also emits a corresponding harness `project` entry to `momentum/harness.json`. The emitted entry need not be fully populated — it must at minimum set the `"role"` field matching the agent being built; all other fields may be omitted entirely (not set to null). The update is additive: existing entries in `momentum/harness.json` are never deleted by agent-builder.
...
8. The architecture document (`_bmad-output/planning-artifacts/architecture.md`) is updated to record: (a) `momentum/harness.json` in the Plugin Root Layout table alongside `momentum/agents.json`, (b) the Read/Write Authority entry for `momentum/harness.json` (sole writers: `agent-builder`, `agent-guidelines`; plugin ships defaults block), and (c) a Decision entry summarizing DEC-029 D3 schema choices.

9. The `momentum/harness.json` file parses as valid JSON with no errors. All required keys described in AC 2 are present in the `defaults` block. The `project` array is present and empty.

10. No existing test, workflow, or configuration file outside `momentum/harness.json`, `skills/momentum/skills/agent-builder/workflow.md`, `skills/momentum/skills/agent-guidelines/workflow.md`, and `_bmad-output/planning-artifacts/architecture.md` is modified by this story.

## Tasks / Subtasks

- [ ] **Task 1 — Create `momentum/harness.json` with plugin-shipped defaults block** (`config-structure`)
  - Create `momentum/harness.json` as a sibling to `momentum/agents.json`
  ...
  - Validate with `jq . momentum/harness.json` — must exit 0 and produce clean output
...
- [ ] **Task 2 — Update `agent-builder` workflow to write harness project entries** (`skill-instruction`)
  ...
  - Immediately after that write, add a step that reads `momentum/harness.json`, checks whether a `project` entry with the matching `"role"` already exists, and if not, appends a stub: `{ "role": "{{role}}", "domain": "{{domain}}" }` to the `project` array
...
- [ ] **Task 3 — Update `agent-guidelines` workflow to write harness project entries** (`skill-instruction`)
  ...
  - Add a check: does `momentum/harness.json` contain a `project` entry with the relevant `"role"`? If no entry exists, write a stub (same shape as Task 2). If entry exists, leave it unchanged
...
- [ ] **Task 4 — Update architecture.md to document `momentum/harness.json`** (`specification`)
  - Read `_bmad-output/planning-artifacts/architecture.md` before editing
  - In the Plugin Root Layout table, add a row for `momentum/harness.json` directly below the `momentum/agents.json` row
  - In the Read/Write Authority table, add a row for `momentum/harness.json`: description "Validation harness profile — `defaults` block (plugin-shipped) + `project` block (per-project overrides)"; sole writers: agent-builder, agent-guidelines; plugin ships defaults block
  - Add a new Decision entry in the Architecture Decisions section, sequentially after the last existing Decision entry (currently Decision 57 / DEC-028). Title it "Decision 58 (DEC-029 D3): harness.json Validation Harness Profile". Content summary: momentum/harness.json is a sibling to agents.json, uses the same defaults/project schema pattern, plugin ships safe defaults (all surfaces "skip"), project block written by agent-builder/agent-guidelines, consumed by e2e-validator and sprint-planning.
  - Verify all cross-references resolve: the agents.json row in both tables, the DEC-029 reference, the path `momentum/harness.json`
...
- `momentum/agents.json` shape:
  ...
  ```
- `momentum/harness.json` shape (this story):
  ```json
  { "defaults": { "<section>": { ... } }, "project": [] }
  ```

The `momentum/` directory currently contains only `agents.json`. This story adds `harness.json` as its only sibling. No other files in that directory are touched.

**Write authority** (per architecture Read/Write Authority table): agent-builder is the sole authorized writer to `momentum/agents.json` project block; by extension (per DEC-029 D3), agent-builder and agent-guidelines are the co-writers of `momentum/harness.json` project block. Plugin ships the defaults block. No other skill writes to either file.
...
**Driver key naming:** DEC-029 D3 uses the term "Skill-invoke" for the skill-invocation driver. In `momentum/harness.json`, this driver is keyed as `"skill"` (a shorter canonical key name). These are the same driver; the key name `"skill"` is the authoritative harness.json identifier.

### Testing Requirements

- **Task 1 (config-structure):** No unit tests. Verify with `jq . momentum/harness.json` (must parse clean). Inspect all required fields manually per AC 9.
...
**`momentum/harness.json` draft structure** (write this exactly, then validate with jq):
...
**agent-guidelines write step (Task 3):** Same pattern. The check happens at whatever step currently writes project-specific outputs for the relevant role. If no such step exists, add a dedicated "Update harness.json project entry" step.

### Project Structure Notes

Files modified by this story:
- `momentum/harness.json` — NEW (create)
...
1. Write `momentum/harness.json` per the draft structure in the Implementation Guide above
2. Verify by inspection:
   - JSON files: validate with `jq . momentum/harness.json` — must exit 0
   - Required fields: all keys described in AC 2 must be present with correct types
   - Paths: `momentum/harness.json` must exist alongside `momentum/agents.json`
...
- [ ] `momentum/harness.json` parses without error (`jq` exit 0)
- [ ] All required fields present with correct types (AC 2 checklist)
- [ ] `momentum/harness.json` is a sibling to `momentum/agents.json`
...
2. Verify cross-references: `momentum/harness.json` path, `momentum/agents.json` row in both tables, DEC-029 decision reference, all section links
...
- [ ] All cross-references to agents.json row, DEC-029, and `momentum/harness.json` path resolve correctly
