...
**Governing spec sections (cited by number from the brief).** Section 7 defines `coverage_disposition` and the `covered-by-composition` path that discharges a story's verification at AVFL/merge via a named integration scenario. §7 step 4 is the build-time skip decision: the QA/verifier consumes the `coverage_disposition` and, if `covered-by-composition`, skips the dedicated run at build time. The actual discharge of the deferred verification debt — running the named integration scenario — is specified in §5 (AVFL-on-merge). The branch in this story is the build-phase fork that honors §7 step 4 by either running the dedicated QA pass at build time (`dedicated-run`) or recording the deferral and skipping the build-time run (`covered-by-composition`), with discharge ownership living in §5.
