...
**Sprint goal field (added 2026-07-13, DEC-039 D1):** `active`, `planning`, and `completed` sprint-record entries carry a `goal` field — a one-to-few-sentence string naming what the sprint delivers. It is captured during sprint-planning (Steps 1–2, Backlog presentation / Story selection) and approved by the developer at the plan gate (Sprint Planning Workflow Step 7 — Developer review). The `goal` field is the anchor read by two downstream mechanisms: the DEC-039 D2 goal-criticality routing matrix (Sprint Orchestration Architecture), which classifies conduct-discovered work as goal-critical or not against this field, and the DEC-039 D3 dual end-gate verdict (Decision 59), which renders a goal-delivered/not-delivered verdict against it. Write authority is unchanged — `momentum-tools.py sprint` is the sole writer, same as the rest of the sprint record.
...
**Dual end-gate verdict (DEC-039 D3):** the end-gate renders **two distinct verdicts**, not one — a **stories-delivered** verdict (the existing per-story ship/dismiss accounting from the Execution model paragraph above) and a **goal-delivered** verdict (whether the sprint's captured `goal`, per the Sprint Tracking Schema `goal` field, was actually achieved). **A sprint can pass the stories-delivered verdict and still fail the goal-delivered verdict** — every planned story can merge cleanly while the goal itself goes undelivered, e.g. because a tier-(d) goal-critical item (per the goal-criticality routing matrix, Sprint Orchestration Architecture) was punted to a new-sprint recommendation rather than folded into the active build. Any **tier-(d) punt of goal-critical discovered work automatically fails the goal verdict** and **obligates a retro root-cause** — the next retro must record why the goal-critical work wasn't sized correctly at planning, feeding the Retro → Planning Handoff section's next-sprint intake. **Implementation status:** conductor-side machinery for the dual verdict is **not yet implemented** — applied manually (developer judgment at the end-gate) until stories land (sprint-planning, conductor, sprint-manager, retro).
